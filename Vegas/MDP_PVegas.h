...
private:
	...
	void PrintInputParameters(int init, int ncalls, int niterations) {
		...
		if (OutputFile != 0 && ME == 0) {
			...
			fprintf(OutputFile,
					"Integration Limits: i\tmin(x[i])\tmax(x[i])\n");
			fprintf(OutputFile,
					"=================================================\n");
			for (j = 0; j < NumberOfDimensions; j++)
				fprintf(OutputFile, "                    %i\t%f\t%f\n", j,
						IntegrationLimitsMin[j], IntegrationLimitsMax[j]);
			fprintf(OutputFile, "\n");
			fflush(OutputFile);
		}
	}
...
public:
	...
	void parallel_loop(double (*function)(double*, void*), double &fb,
			double &f2b) {
	...
	double Integrate(double (*function)(double*, void*), int init = 0,
			int ncalls = 1000, int niterations = 100) {
		int i, j, k;
