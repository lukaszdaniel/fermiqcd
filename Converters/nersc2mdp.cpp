...
public:
	...
	;
	int initialize(int x1, int x2, int x3, int a = 1, int b = 1, int c = 1,
			int d = 1) {
	...
	;
	Complex &operator()(int x1, int x2, int x3, int a = 0, int b = 0, int c = 0,
			int d = 0) {
		return m[(((((x1 * dim[1] + x2) * dim[2] + x3) * dim[3] + a) * dim[4]
				+ b) * dim[5] + c) * dim[6] + d];
	}
	;
};
...
void read_t_gauge(short_field &U, FILE* file, char precision, char swap,
		int rows = 2) {
				...
				for (muf = 1; muf < 5; muf++) { // ukqcd indices 1, 2, 3, 4
					...
					for (a = 0; a < rows; a++)
						for (b = 0; b < (3); b++) {
							...
							// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
							if (precision == 'F') {
								U(x1, x2, x3, mu, a, b) = Complex(
										((float *) buffer)[buf_index],
										((float *) buffer)[buf_index + 1]);

							} else {
								U(x1, x2, x3, mu, a, b) = Complex(
										((double *) buffer)[buf_index],
					...
					if (rows == 2) {
						...
						U(x1, x2, x3, mu, 2, 0) = conj(
								U(x1, x2, x3, mu, 0, 1)
										* U(x1, x2, x3, mu, 1, 2)
										- U(x1, x2, x3, mu, 1, 1)
												* U(x1, x2, x3, mu, 0, 2));
						U(x1, x2, x3, mu, 2, 1) = conj(
								U(x1, x2, x3, mu, 1, 0)
										* U(x1, x2, x3, mu, 0, 2)
										- U(x1, x2, x3, mu, 0, 0)
												* U(x1, x2, x3, mu, 1, 2));
						U(x1, x2, x3, mu, 2, 2) = conj(
								U(x1, x2, x3, mu, 0, 0)
										* U(x1, x2, x3, mu, 1, 1)
										- U(x1, x2, x3, mu, 1, 0)
												* U(x1, x2, x3, mu, 0, 1));
 ...
 */
void read_t_prop(short_field &S, char fileprefix[], char precision, char swap,
		int time) {
	...
	for (source_spin = 0; source_spin < 4; source_spin++) {
		for (source_colour = 0; source_colour < 3; source_colour++) {
			...
			bytes_read = fread(buffer, sizeof(unsigned char), bytes_to_read,
					fp);
			...
			if (swap == 'Y')
				if (precision = 'F') {
					...
					block_swap_double((double *) buffer,
							bytes_to_read / sizeof(double));
				}
				...
				for (x2 = 0; x2 < nx[2]; x2++)
					for (x1 = 0; x1 < nx[1]; x1++) {
						...
						for (sink_spin = 0; sink_spin < 4; sink_spin++) {
							for (sink_colour = 0; sink_colour < 3;
									sink_colour++) {

								if (precision == 'F') {
									S(x1, x2, x3, sink_spin, source_spin,
											sink_colour, source_colour) =
											Complex(
													((float *) buffer)[buffer_index],
													((float *) buffer)[buffer_index
															+ 1]);
								} else {
									S(x1, x2, x3, sink_spin, source_spin,
...
int main(int argc, char **argv) {
	...
	if (argc < 4) {
		printf("Sample usage:\n\n");
		printf("nersc2mdp -skip N -gauge 16x8x8x8FN input_prefix output\n");
	...
	cout << argv[4] << endl;
	char precision, swap;
	sscanf(argv[4], "%ix%ix%ix%i:%i%c%c", nx, nx + 1, nx + 2, nx + 3, &rows,
			&precision, &swap);
