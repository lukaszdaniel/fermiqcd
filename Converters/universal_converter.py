...
def reunitarize(s):
    (a1re, a1im, a2re, a2im, a3re, a3im, b1re, b1im, b2re, b2im, b3re,
     b3im) = s
...
class MilcField:
    ...
    def get_header(self):
        ...
        if not len(header) == 96:
            raise SyntaxError("file too small")
        self.endianess = "<"
        ...
        if not milc_head[0] == 20103:
            raise SyntaxError
        self.lattice_size = [
            milc_head[4], milc_head[1], milc_head[2], milc_head[3]
        ]
        self.offset = 96
    ...
    def convert(self, filename):
        ...
        link_format_out = "<" + self.precision_target * (9 * 2
                                                         )  # 2 for complex
        if self.precision == "f":
            link_size = 9 * 8
        else:
            ...
            for p1 in range(n1):
                for p2 in range(n2):
                    for p3 in range(n3):
                        ...
                        ofile.seek(ooffset + site_size *
                                   (p[Z] + nz * (p[Y] + ny *
                                                 (p[X] + nx * p[T]))))
                        for mu in self.link_order:
                            site[mu] = struct.unpack(
                                self.endianess + link_format,
                                ifile.read(link_size))
                        for mu in [T, X, Y, Z]:
                            ofile.write(struct.pack(link_format_out,
                                                    *site[mu]))
...
class QioField:
    ...
    def get_header(self):
        ...
        while True:
            ...
            if not len(header) == 144:
                break
            lime_head = struct.unpack("!2i1q128s", header)
            if not lime_head[0] == 1164413355:
                raise SyntaxError
            data = lime_head[2]
            print("Lime block:", data, lime_head[3])
            padding = (8 - (data % 8)) % 8
            if lime_head[3][:16] == "ildg-binary-data":
                self.offset = 144 + offset
            if lime_head[3][:11] == "ildg-format":
                regex = re.compile(
                    "\<lx\>(?P<lx>\d+)\</lx\>\<ly\>(?P<ly>\d+)\</ly\>\<lz\>(?P<lz>\d+)\</lz\>\<lt\>(?P<lt>\d+)\</lt\>"
                )
    ...
    def get_header(self):
        ...
        while True:
            ...
            if lime_head[3][:11] == "ildg-format":
                ...
                print("Lattice size from header:", self.lattice_size)
            if lime_head[3][:18] == "scidac-binary-data":
                self.offset = 144 + offset
...
class Nersc3x3Field:
    ...
    def get_header(self):
        ...
        if self.offset < len(key):
            raise SyntaxError("not in NERSC format")
        header = header[:self.offset]
        print(header)
        self.lattice_size = [
            int(
                re.compile("DIMENSION_4\s*=\s*(?P<t>\d+)").search(
                    header).group("t")),
            int(
                re.compile("DIMENSION_1\s*=\s*(?P<x>\d+)").search(
                    header).group("x")),
            int(
                re.compile("DIMENSION_2\s*=\s*(?P<y>\d+)").search(
                    header).group("y")),
            int(
                re.compile("DIMENSION_3\s*=\s*(?P<z>\d+)").search(
                    header).group("z")),
        ]
                ...
                for p2 in range(n2):
                    for p3 in range(n3):
                        ...
                        for mu in self.link_order:
                            ...
                            for i, r in enumerate(site[mu]):
                                if abs(r) > 1:
                                    raise SyntaxError("Invalid Format %f" % r)
                        ofile.seek(ooffset + site_size *
                                   (p[Z] + nz * (p[Y] + ny *
                                                 (p[X] + nx * p[T]))))
                        for mu in [T, X, Y, Z]:
...
def universal_converter(path, formats):
    ...
    if not done:
        raise RuntimeError("failure to convert " + file)
...
class ILDGPropField:
    ...
    def convert(self, filename):
        ...
        spinor_format_out = "<" + self.precision_target * (16 * 9 * 2
                                                           )  # 2 for complex
        if self.precision == "f":
            spinor_size = 16 * 9 * 8
        else:
            ...
            for p1 in range(n1):
                for p2 in range(n2):
                    for p3 in range(n3):
                        ...
                        ofile.seek(ooffset + spinor_size *
                                   (p[0] + nt * (p[Z] + nz * (p[Y] + ny *
                                                              (p[X])))))
...
def main():
    ...
    parser.add_option(
        ...
        dest="format",
        default="ALL",
        help="any of the supported formats (%s)" % ",".join(f[0]
                                                            for f in formats),
    )
