...
void read_t_gauge(short_field &U, char fileprefix[], char precision, char swap,
		int time) {
			...
			for (x1 = 0; x1 < nx[1]; x1++) {
				for (muf = 1; muf < 5; muf++) { // ukqcd indices 1, 2, 3, 4
					...
					U(x1, x2, x3, mu, 2, 0) = conj(
							U(x1, x2, x3, mu, 0, 1) * U(x1, x2, x3, mu, 1, 2)
									- U(x1, x2, x3, mu, 1, 1)
											* U(x1, x2, x3, mu, 0, 2));
					U(x1, x2, x3, mu, 2, 1) = conj(
							U(x1, x2, x3, mu, 1, 0) * U(x1, x2, x3, mu, 0, 2)
									- U(x1, x2, x3, mu, 0, 0)
											* U(x1, x2, x3, mu, 1, 2));
					U(x1, x2, x3, mu, 2, 2) = conj(
							U(x1, x2, x3, mu, 0, 0) * U(x1, x2, x3, mu, 1, 1)
									- U(x1, x2, x3, mu, 1, 0)
											* U(x1, x2, x3, mu, 0, 1));
				}
			}
