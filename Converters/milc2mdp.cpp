...
int main(int argc, char **argv) {
	...
	if (strcmp(argv[1], "-gauge") == 0) {
		...
		if ((MILC_header.dims[0] != nx[1]) && (MILC_header.dims[1] != nx[2])
				&& (MILC_header.dims[2] != nx[3])
				&& (MILC_header.dims[3] != nx[0])) {

			printf("%xx%xx%xx%x => Incorrect dimensions!\n",
					MILC_header.dims[3], MILC_header.dims[0],
					MILC_header.dims[1], MILC_header.dims[2]);
		...
		for (x0 = 0; x0 < nx[0]; x0++) {
			for (x3 = 0; x3 < nx[3]; x3++)
				for (x2 = 0; x2 < nx[2]; x2++)
					for (x1 = 0; x1 < nx[1]; x1++)
						for (mu = 1; mu <= 4; mu++)
							if(fread(&U(x1, x2, x3, mu % 4, 0, 0), matrix_size, 1,
									MILC_fp) != matrix_size) {
								error("Error while reading from file");
							}
		...
		printf(
				"I am sorry, but conversion of propagators is not implemented yet!\n");
	};
}
;
