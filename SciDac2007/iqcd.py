...
allowed_commands = {
    "cold": [["TxXxYxZ", "\d+x\d+x\d+x\d+", "16x4x4x4"], ["nc", "\d+", "3"]],
    "hot": [["TxXxYxZ", "\d+x\d+x\d+x\d+", "16x4x4x4"], ["nc", "\d+", "3"]],
    "load": [
        ["filename", "[a-zA-Z0-9_/.\-]*", None],
        ["precision", "(?:float)|(?:double)", "float"],
    ],
    "lload": [
        ["filename", "[a-zA-Z0-9_/.\-\*]*", "gauge.*.mdp"],
        ["precision", "(?:float)|(?:double)", "float"],
    ],
    ...
    "topological_charge": [
        ["filename", "[a-zA-Z0-9_/.\-]*", "topological_charge_*.vtk"],
        ["t", "\d", "-1"],
    ],
}
...
comments = {
    "cold":
    'creates a cold gauge configuration, i.e. all links U(x,mu)=1\nexmaple: "-cold nc=3 TxXxYxZ=10x4x4x4"',
    "hot":
    'creates a hot gauge configuration, i.e.  all links U(x,mu)=random.SU(nc)\nexmaple: "-hot nc=3 TxXxYxZ=10x4x4x4"',
    "load":
    "loads an existing gauge configuration in mdp fermiqcd format\ndetermines lattice size from file",
    "lload":
    "lloads in a loop an existing gauge configuration in mdp fermiqcd format\ndetermines lattice size from file",
    "save_partitioning":
    "save a vtk file showing parallel partitioning information",
    "heatbath":
    "performs a number of heatbath steps using the WilsonGaugeAction",
    "save":
    "saves current gauge configuration",
    "plaquette":
    "compute average plquette",
    "ape_smear":
    "APE smearing as defiend in ref...",
    "coulomb_gauge_fix":
    "Coulomb Gauge Fixing algorithm as defiend in ref...\n(can also do z3 fixing if z3=1 as in ref...)",
    "landau_gauge_fix":
    "Landau Gauge Fixing algorithm as defiend in ref...",
    "topological_charge":
    "saves a vtk showing the topological charge as defined in ref...\n(one should ape_smear first)",
}
    ...
    for s in commands:
        if s.command == "loop":
            ...
            for a, b, c in arguments:
                ...
                if a not in s.args:
                    if c == None:
                        errors.append("+%s error: missing %s=... argument" %
                                      (s.command, a))
                    else:
                        s.args[a] = c
                        warnings.append(
                            "+%s warning: assuming default argument %s=%s" %
                            (s.command, a, str(c)))
                elif re.sub(b, "", s.args[a]) != "":  # fix this line
                    errors.append("+%s error: invalid argument %s=%s" %
                                  (s.command, a, s.args[a]))
            for a in s.args:
                if a not in d:
                    errors.append("+%s error: unkown argument %s=..." %
                                  (s.command, a))
        if s.command in ["cold", "hot"] and k > 0:
            errors.append("+%s error: must be the first algorithm!" %
                          (s.command))
        k += 1
        ...
        if not have_gauge:
            ...
            if s.command == "load":
                ...
                if s.args["precision"] == "float":
                    program += '   U.load_as_float("%s");\n' % s.args[
                        "filename"]
                if s.args["precision"] == "double":
                    program += '   U.load_as_double("%s");\n' % s.args[
                        "filename"]
                have_gauge = True
        else:
            if s.command == "loop":
            ...
            if s.command == "load":
                if s.args["precision"] == "float":
                    program += SPACE + 'U.load_as_float("%s");\n' % s.args[
                        "filename"]
                if s.args["precision"] == "double":
                    program += SPACE + 'U.load_as_double("%s");\n' % s.args[
                        "filename"]
            if s.command == "lload":
                if s.args["precision"] == "float":
                    program += SPACE + 'U.load_as_float(glob("%s")[i%i]);' % (
                        s.args["filename"],
                        len(loops) - 1,
                    )
                if s.args["precision"] == "double":
                    program += SPACE + 'U.load_as_double(glob("%s")[i%i]);' % (
                        s.args["filename"],
                        len(loops) - 1,
                    )
            if s.command == "save_partitioning":
                program += SPACE + 'save_partitioning_vtk(spacetime,"%s");' % (
                    s.args["filename"])  # NOT IMPLEMENTED
            if s.command == "save":
            ...
            if s.command == "heatbath":
                ...
                program += (SPACE +
                            "WilsonGaugeAction::heatbath(U,coeff,%s);" %
                            s.args["steps"])
            if s.command == "ape_smear":
                program += SPACE + "ApeSmearing::smear(U,%s,%s,%s);" % (
                    s.args["alpha"],
                    s.args["steps"],
                    s.args["cooling_steps"],
                )
            if s.command == "coulomb_gauge_fix":
                program += (
                    SPACE +
                    "GaugeFixing::fix(U,GaugeFixing::Coulomb,%s,%s,%s,%s);" % (
                        s.args["steps"],
                        s.args["precision"],
                        s.args["boost"],
                        s.args["z3"],
            ...
            if s.command == "landau_gauge_fix":
                program += (
                    SPACE +
                    "GaugeFixing::fix(U,GaugeFixing::Landau,%s,%s,%s);" %
                    (s.args["steps"], s.args["precision"], s.args["boost"]))
            if s.command == "topological_charge":
                program += SPACE + '{float tc=topological_charge_vtk(U,"%s",%s);' % (
                    s.args["filename"],
                    s.args["t"],
                )  # NOT IMPLEMENTED
...
def menu():
    if len(sys.argv) == 1:
        ...
        print(
            '"python qcd.py help -heatbath" for help about the heatbath algorithm'
        )
        print(
            '"python qcd.py code +cold TxXxYxZ=10x4x4x4 nc=3" to create a program that makes one cold gauge configuration'
        )
        print(
            '"python qcd.py compile filename.cpp" to compile filename.cpp (for those who hate make like me)'
        )
