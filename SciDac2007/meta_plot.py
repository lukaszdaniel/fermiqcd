...
class Plot:
    ...
    def show_tk(self):
        ...
        self.canvas.create_window(self.width,
                                  self.height,
                                  anchor="se",
                                  window=self.button)
    ...
    def abcd(self):
        if self.minx < 0:
            minx = -self.dx * int(-self.minx / self.dx + 0.99)
        else:
            minx = self.dx * int(self.minx / self.dx - 0.99)
        maxx = self.dx * int(self.maxx / self.dx + 0.99)
        if self.miny < 0:
            miny = -self.dy * int(-self.miny / self.dy + 0.99)
        else:
            miny = self.dy * int(self.miny / self.dy - 0.99)
    ...
    def plot_axis(self):
        try:
            if self.x1 != self.x0:
                self.minx, self.maxx = min(self.x0,
                                           self.x1), max(self.x0, self.x1)
            if self.y1 != self.y0:
                self.miny, self.maxy = min(self.y0,
                                           self.y1), max(self.y0, self.y1)
            if self.x1 == self.x0 and self.y1 == self.y0:
                raise Exception
            self.minx, self.dx, self.maxx = self.round(self.minx, self.maxx,
                                                       10)
            self.miny, self.dy, self.maxy = self.round(self.miny, self.maxy,
                                                       10)
            if self.startdate and self.dx < 1:
                self.dx = 1.0
        except:
            for legend, color, data in self.sets_plot:
                self.find_axis(data)
            for legend, color, data, width in self.sets_hist:
                self.find_axis_hist(data)
            pass
        self.legend_counter = 0
        minx = self.minx
        miny = self.miny
        ...
        while x0 <= maxx + dx / 2:
            ...
            if not self.startdate:
                ...
                date = (self.startdate +
                        datetime.timedelta(days=int(x0))).strftime("%d%b\n%Y")
                self.create_text(a + b * x0,
                                 c + d * y0 + 5,
                                 text=date,
                                 anchor="n",
    ...
    def pick_color(self):
        if self.legend_counter == 0:
            return "#6699FF"
        if self.legend_counter == 1:
            return "#FF9966"
        if self.legend_counter == 2:
            return "#66FF99"
        if self.legend_counter == 3:
            return "#99FF66"
        if self.legend_counter == 4:
            return "#FF6699"
        if self.legend_counter == 5:
            return "#9966FF"
        return self.fg
    ...
    def plot_(self, data, color=None, legend=""):
        ...
        if len(data[0]) > 2:
            rx = ry = min(max(self.minr, int(self.width / 20 / len(data))),
                          self.maxr)
            for i in range(len(data)):
    ...
    def rectangle(self, x0, y0, x1, y1, color="white", legend="", text=""):
        if not color:
            color = self.pick_color()
        a, b, c, d, minx, maxx, miny, maxy = self.abcd()
        id = self.create_rectangle(a + b * x0,
                                   c + d * y0,
                                   a + b * x1,
                                   c + d * y1,
                                   fill=color)
...
def chi_squared(set_exp, set_th):
    ...
    sum = 0.0
    for i in range(n):
        if abs(set_exp[i][0] - set_th[i][0]) > 0.01:
            raise SyntaxError
...
def test_plot():
    ...
    p.plot([(5, 2, -1, 1), (10, 2, -1, 1), (20, 3, -1, 1), (40, 4, -1, 1)],
           legend="set1")
...
def test_gauss():
    ...
    for i in range(100000):
        set.append(gauss(mu, sigma))
    data1 = bin(set, 20, 0, 40)
    data2 = []
    f = (lambda x: 1.0 / sqrt(2.0 * 3.1415) / sigma * exp(-(
        (x - mu)**2) / 2.0 / sigma**2))
    for x, y in data1:
        data2.append((x, f(x)))
