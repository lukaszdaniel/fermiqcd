...
class IFit:
    ...
    def __init__(self,
        ...
        {'a':1.0,'b':1.0}
        """
        if not symbols and len(points[0]) == 4:
            symbols = ["x"]
        if not symbols and len(points[0]) == 5:
            symbols = ["x", "y"]
        if not symbols and len(points[0]) == 6:
            symbols = ["x", "y", "z"]
        if not symbols and len(points[0]) == 7:
            symbols = ["x", "y", "z", "t"]
        if not symbols:
            raise Exception
        ...
        self.undefined_symbols = [
            k for k in e if k and k not in self.locals and not k in symbols
        ]
    ...
    def iterative_fit(self, filename="ifit_scatter.csv", **variables):
        ...
        writer.writerows(
            [list(item[0].values()) for item in variables_samples])
    ...
    def plot2d(self, key1="a", key2="b", scale1=0.1, scale2=0.1):
        ...
        for k in range(len(keys)):
            if keys[k] == key1:
                i, vi = k, self.last_variables[key1]
            if keys[k] == key2:
                j, vj = k, self.last_variables[key2]
        ax = pylab.arange(vi * (1.0 - 10 * scale1), vi * (1.0 + 12 * scale1),
                          vi * scale1)[:21]
        ay = pylab.arange(vj * (1.0 - 10 * scale2), vj * (1.0 + 12 * scale2),
                          vj * scale2)[:21]
        ...
        for x in ax:
            ...
            for y in ay:
                ...
                for ik in range(nv):
                    for jk in range(nv):
                        least_squares += (0.5 * dv[ik] * dv[jk] *
                                          self.last_hessian[ik][jk])
    ...
    def save_fit_trail(self, filename):
        keys = [
            k for k in list(self.last_trail[0].keys())
            if not k == "[least_squares]"
        ]
        keys.sort()
    ...
    def save_fit(self, filename):
        ...
        expression = (self.expression + "@" + ",".join(
            ["%s=%g" % (k, v) for k, v in list(self.last_variables.items())]))
...
def test_correlated_ifit():
    ...
    ifit = IFit("(a0*(x==0)+a1*(x==1)+a2*(x==2))*sin(y)+b*y",
                points,
                symbols=["x", "y"])
...
def main_ifit():
    ...
    try:
        ...
        if ifit.scatter_points:
            ifit.iterative_fit(options.input_prefix + "_scatter.csv",
                               **variables)
        for key, value in list(variables.items()):
            print("%s = %g" % (key, value))
        ...
        for item in options.extrapolations:
            ...
            try:
                e = ifit.extrapolate_with_errors(**coordinates)
            except:
