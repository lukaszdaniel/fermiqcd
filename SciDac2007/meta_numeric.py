...
def Jacobi(A, checkpoint):
    ...
    while state:
        ...
        for i in range(k):
            S[i][k], S[i][
                h] = c * S[i][k] - s * S[i][h], s * S[i][k] + c * S[i][h]
            pass
        for i in range(k + 1, h):
            S[k][i], S[i][
                h] = c * S[k][i] - s * S[i][h], s * S[k][i] + c * S[i][h]
            pass
        for i in range(h + 1, n):
            S[k][i], S[h][
                i] = c * S[k][i] - s * S[h][i], s * S[k][i] + c * S[h][i]
            pass
        for i in range(n):
            E[k][i], E[h][
                i] = c * E[k][i] - s * E[h][i], s * E[k][i] + c * E[h][i]
            pass
