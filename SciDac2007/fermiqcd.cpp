...
public:
	...
	FermiQCD() {
		...
		prefix = make_prefix();
		if (ME == 0)
			system((string("mkdir ") + prefix).c_str());
		if (ME == 0)
	...
	void init_cold(int LT, int LX, int LY, int LZ, int nc) {
		...
		this->L[3] = LZ;
		this->nc = nc;
		os << "making an lattice TxXxYxZ=" << LT << "x" << LX << "x" << LY
				<< "x" << LZ << endl;
	...
	void init_load(string filename) {
		...
		os << "making an lattice TxXxYxZ=" << this->L[0] << "x" << this->L[1]
				<< "x" << this->L[2] << "x" << this->L[3] << endl;
		this->plattice = new mdp_lattice(4, L);
		os << "loading gauge configuration " << filename << " into U with nc="
				<< nc << endl;
		this->pU = new gauge_field(*plattice, nc);
		if (precision == 4)
	...
	void wilson_heatbath(float beta, int steps = 1) {
		...
		for (int step = 0; step < steps; step++) {
			(*this) << "WilsonGaugeAction::heatbath(beta=" << beta << ") step="
					<< step << endl;
	...
	void ape_smearing(float alpha = 0.7, int iterations = 20,
	...
	void coulomb_gauge_fix(int iterations = 100) {
		...
		float boost = 1;
		gaugefixing_stats stats = GaugeFixing::fix(*pU, GaugeFixing::Coulomb,
				iterations, precision, boost, false);
		(*this) << "done!\n";
	}
	void coulomb_z3_gauge_fix(int iterations = 100) {
		...
		float boost = 1;
		gaugefixing_stats stats = GaugeFixing::fix(*pU, GaugeFixing::Coulomb,
				iterations, precision, boost, true);
		(*this) << "done!\n";
	}
	void landau_gauge_fix(int iterations = 100) {
		...
		float precision = 1e-6;
		float boost = 1;
		gaugefixing_stats stats = GaugeFixing::fix(*pU, GaugeFixing::Landau,
				iterations, precision, boost, false);
		(*this) << "done!\n";
	}
