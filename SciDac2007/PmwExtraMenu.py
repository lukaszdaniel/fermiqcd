...
class ExtraMenu:
    ...
    def post(self, event=None, activate_button=True):
        ...
        if not self._is_submenu:
            self._add_binding(self._root, "<ButtonPress-1>", self._close_event,
                              "+")
            self._add_binding(self._root, "<Configure>", self._reconfigure,
                              "+")
        ...
        for wdict in self._wdicts:
            ...
            if wtype == "button":
                ...
                self._add_binding(w, "<Return>", self._child_post_and_activate,
                                  "+")
    ...
    def _reconfigure(self, event):
        ...
        old_b_x, old_b_y = re.match(r"\d+x\d+([+-]\d+)([+-]\d+)",
                                    self._self_geom).groups()
        b_x, b_y = re.match(r"\d+x\d+([+-]\d+)([+-]\d+)",
                            self.button.winfo_geometry()).groups()
        # get the co_ordinates of our frame (these are relative to the
        # root window
        f_wh, f_x, f_y = re.match(r"(\d+x\d+)([+-]\d+)([+-]\d+)",
                                  self._frame_geom).groups()
        ...
        self._frame_geom = "%s%+d%+d" % (f_wh, int(f_x) + dx, int(f_y) + dy)
    ...
    def _child_reconfigure(self, dx, dy):
        ...
        amount"""
        x, y, anchor = self._locate_place_point()
        self._menu_frame.place_configure(x=x + dx, y=y + dy, anchor=anchor)
        if self._child is not None:
            self._child._child_reconfigure(dx, dy)
    ...
    def _link_arrows(self, do_binding):
        ...
        for i in range(len(self._selectables)):
            ...
            if wtype != "widget":
                w.wnext = self._selectables[(i + 1) % len(self._selectables)]
    ...
    def _prepare_selectable(self, w, wdict, command=None, allow_active=False):
    ...
    def _is_my_window(self, x, y):
        ...
        return x >= nwx and x <= (nwx + ww) and y >= nwy and y <= (nwy + wh)
    ...
    def _add_widget(self, wtype, *args, **kw):
        ...
        if wtype != "widget":
            ...
            if len(self._wdicts) > 0 and self._wdicts[0]["type"] == "widget":
                return -1
        else:
    ...
    def enable_widget(self, widget_id):
        ...
        self._selectables = (
            [sw for sw in self._selectables if sw.wdict in self._wdicts[:i]] +
            [w] +
            [sw for sw in self._selectables if sw.wdict in self._wdicts[i:]])
...
if __name__ == "__main__":
    ...
    def dir_setup(w):
        ...
        if sort_dirs.get():
            if not caseless.get():
                dirs.sort(lambda x, y: cmp(len(x), len(y)) or cmp(x, y))
            else:
    ...
    def files_setup(w):
        ...
        if sort_files.get():
            if not caseless.get():
                ...
                files.sort(lambda x, y: cmp(len(x), len(y)) or cmp(x, y))
    ...
    emrcmb = ExtraMenu(None,
                       root,
                       relief=RAISED,
                       borderwidth=2,
                       is_submenu=True)
    emrcmb.add_special_widget(
        ...
        for item in itemlist:
            if item[1] == "add_command":
                cmd = ("%s.%s( text = '%s', relief = RAISED,"
                       "borderwidth = 0 %s )" % tuple([menu] + item[1:]))
            if item[1] == "add_label":
                cmd = ("%s.%s( text = '%s', relief = RAISED,"
                       "borderwidth = 0 %s )" % tuple([menu] + item[1:]))
    ...
    def toggle_sub_auto():
        ...
        emr.set_widget_options(r_menu_items[4][0],
                               text=txt,
                               auto_post=auto_post_sub.get())

    r_menu_items = [
        #     command, [submenu], text, auto_post/var, val, extra args
        [0, "add_command", "One", ""],
        [0, "add_submenu", "emrcmb", "Choose Dir", "True", ", underline = 7"],
        [0, "add_submenu", "emropt", "Choose File", "True", ", underline = 7"],
        [0, "add_separator"],
    ...
    def add_button_5():
        ...
        button5 = emr.add_command(text="Five",
                                  relief=RAISED,
                                  borderwidth=0,
                                  insert_before=ind)
        print(button5)
