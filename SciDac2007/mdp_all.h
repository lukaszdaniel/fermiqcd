...
public:
	...
	int Connect(int sfd, int timeout = 0) {
		if (!timeout) {
			...
			if (ret < 0) {
				...
				if (getsockopt(sfd, SOL_SOCKET, SO_ERROR, (void*) &valopt, &lon)
						< 0 || valopt)
					return ECONNREFUSED;
				if (ret > 0)
					return 0;
	...
	int sendTo(int sfd, void* data, int size, int options = 0) {
		return sendto(sfd, data, size, options, (struct sockaddr*) &address,
				(socklen_t) sizeof(address));
	}
	int recvFrom(int sfd, void* data, int size, int options = 0) {
		socklen_t address_size = sizeof(address);
		return recvfrom(sfd, data, size, options, (struct sockaddr*) &address,
...
int setSocketRecvMulticast(int sfd, string from) {
	...
	mreq.imr_multiaddr.s_addr =
			InternetAddress(from, 0).address.sin_addr.s_addr;
...
void Thread::threadStart() {
	pthread_create(&_thread_number, &_thread_attributes, thread_function,
			(void*) this);
}
