...
__stringBody = ("def %(method)s(this, *args, **kw): return " +
...
__funcBody = ("def %(method)s(this, *args, **kw): return " +
...
def forwardmethods(fromClass, toClass, toPart, exclude=()):
    ...
    if type(toPart) != bytes:
        ...
        # It's not a valid type
            ...
            raise TypeError(
                "toPart must be attribute name, function or method")
...
class MegaArchetype:
    ...
    def __init__(self, parent=None, hullClass=None):
        ...
        if hullClass is None:
            ...
            # Create the hull.
            self._hull = self.createcomponent("hull", (), None, hullClass,
                                              (parent, ))
    ...
    def createcomponent(
        ...
        **kw,
        ...
        self.__componentInfo[componentName] = (
            widget,
            widget.configure,
            componentClass,
            widget.cget,
    ...
    def createlabel(self, parent, childCols=1, childRows=1):
        ...
        label = self.createcomponent("label", (), None, tkinter.Label,
                                     (parent, ))
    ...
    def configure(self, option=None, **kw):
                ...
                if index >= 0:
                    ...
                    if componentInfo_has_key(component):
                        ...
                        if (len(componentConfigFuncs) == 0
                                and component not in self._dynamicGroups):
        ...
        list(
            map(
                apply,
                list(indirectOptions.keys()),
                ((), ) * len(indirectOptions),
                list(indirectOptions.values()),
            ))
...
class MegaToplevel(MegaArchetype):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._hull._Pmw_WM_DELETE_name = self.register(self._userDeleteWindow,
                                                       needcleanup=0)
...
class MegaWidget(MegaArchetype):
    ...
    def __init__(self, parent=None, **kw):
        # Define the options for this megawidget.
        optiondefs = (("hull_class", self.__class__.__name__, None), )
        self.defineoptions(kw, optiondefs)
    ...
    for (window, winInfo) in list(_toplevelBusyInfo.items()):
        if (window.state() != "withdrawn" and not winInfo["isBusy"]
            ...
            if _disableKeyboardWhileBusy:
                ...
                # keyboard events.
                winInfo["windowFocus"] = window.tk.call(
...
def drawarrow(canvas, color, direction, tag, baseOffset=0.25, edgeOffset=0.15):
    ...
    bw = string.atoi(canvas["borderwidth"]) + string.atoi(
        canvas["highlightthickness"])
...
def _reporterror(func, args):
    ...
    if type(args) == tuple and len(args) > 0 and hasattr(args[0], "type"):
        eventArg = 1
    else:
        eventArg = 0
...
class Dialog(MegaToplevel):
    ...
    def __init__(self, parent=None, **kw):
        ...
        if width > 0:
            self._separator = self.createcomponent(
                "separator",
                (),
                None,
                tkinter.Frame,
    ...
    def _doCommand(self, name=None):
        if (name is not None and self.active()
                and grabstacktopwindow() != self.component("hull")):
...
def ymdtojdn(year, month, day, julian=-1, papal=1):
    ...
    if julian:
        return (367 * year - _cdiv(7 * (year + 5001 + _cdiv(
            (month - 9), 7)), 4) + _cdiv(275 * month, 9) + day + 1729777)
    else:
        return (
            (day - 32076) +
            _cdiv(1461 * (year + 4800 + _cdiv((month - 14), 12)), 4) +
            _cdiv(367 * (month - 2 - _cdiv(
                (month - 14), 12) * 12), 12) - _cdiv((3 * _cdiv(
                    (year + 4900 + _cdiv(
                        (month - 14), 12)), 100)), 4) + 1)  # correction by rdg
...
class Balloon(MegaToplevel):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._label = self.createcomponent("label", (), None, tkinter.Label,
                                           (interior, ))
        self._label.pack()
    ...
    def bind(self, widget, balloonHelp, statusHelp=None):
        ...
        enterId = widget.bind(
            "<Enter>",
            lambda event, self=self, w=widget, sHelp=statusHelp, bHelp=
            balloonHelp: self._enter(event, w, sHelp, bHelp, 0),
        )
        ...
        motionId = widget.bind(
            "<Motion>",
            lambda event=None, self=self, statusHelp=statusHelp: self.
    ...
    def tagbind(self, widget, tagOrItem, balloonHelp, statusHelp=None):
        ...
        if statusHelp is None:
            statusHelp = balloonHelp
        enterId = widget.tag_bind(
            tagOrItem,
            "<Enter>",
            lambda event, self=self, w=widget, sHelp=statusHelp, bHelp=
            balloonHelp: self._enter(event, w, sHelp, bHelp, 1),
        )
        motionId = widget.tag_bind(
            tagOrItem,
            "<Motion>",
            lambda event=None, self=self, statusHelp=statusHelp: self.
            showstatus(statusHelp),
        ...
        buttonId = widget.tag_bind(tagOrItem, "<ButtonPress>",
                                   self._buttonpress)
        ...
        if not hasattr(widget, "_Pmw_BalloonBindIds"):
            widget._Pmw_BalloonBindIds = {}
        widget._Pmw_BalloonBindIds[tagOrItem] = (enterId, motionId, leaveId,
                                                 buttonId)

    def tagunbind(self, widget, tagOrItem):
        if hasattr(widget, "_Pmw_BalloonBindIds"):
            if tagOrItem in widget._Pmw_BalloonBindIds:
                (enterId, motionId, leaveId,
                 buttonId) = widget._Pmw_BalloonBindIds[tagOrItem]
    ...
    def _enter(self, event, widget, statusHelp, balloonHelp, isItem):
        ...
        if (not buttonPressed and balloonHelp is not None
                and self["state"] in ("balloon", "both")):
            if self._timer is not None:
                self.after_cancel(self._timer)
                self._timer = None
            ...
            self._timer = self.after(
                self["initwait"],
                lambda self=self, widget=widget, help=balloonHelp, isItem=
                isItem: self._showBalloon(widget, help, isItem),
            )
...
class ButtonBox(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        if self["labelpos"] is None:
            ...
            self._buttonBoxFrame = self.createcomponent(
                "frame", (), None, tkinter.Frame, (interior, ))
    ...
    def index(self, index, forInsert=0):
        ...
        if type(index) == int:
            ...
            raise ValueError('bad index "%s": must be %s' %
                             (index, validValues))
    ...
    def insert(self, componentName, beforeComponent=0, **kw):
        ...
        button = self.createcomponent(
            *(componentName, (), "Button", tkinter.Button,
              (self._buttonBoxFrame, )),
            **kw,
        )
        ...
        if horizontal:
            ...
            self._buttonBoxFrame.grid_columnconfigure(numButtons * 2 + 2,
                                                      weight=1)
        else:
            button.grid(
                column=0,
            ...
            self._buttonBoxFrame.grid_rowconfigure(numButtons * 2 + 2,
                                                   weight=1)
...
class EntryField(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        # Create the components.
        interior = self.interior()
        self._entryFieldEntry = self.createcomponent("entry", (), None,
                                                     tkinter.Entry,
                                                     (interior, ))
        ...
        self._entryFieldEntry.bindtags(("EntryFieldPre", ) +
                                       self._entryFieldEntry.bindtags() +
                                       ("EntryFieldPost", ))
    ...
    def _validate(self):
        ...
        if "stringtovalue" in dict:
            ...
            self._checkValidateFunction(strFunction, "stringtovalue",
                                        stringtovalue)
        else:
    ...
    def _preProcess(self):
        ...
        if self._entryFieldEntry.selection_present():
            self._previousSel = (
                self._entryFieldEntry.index("sel.first"),
    ...
    def _checkValidity(self):
        ...
        if valid == ERROR:
            ...
            if self._previousText is not None:
                ...
                if self._previousSel is not None:
                    self._entryFieldEntry.selection_range(
                        self._previousSel[0], self._previousSel[1])
        ...
        if valid == OK:
            ...
            if self.normalBackground is None:
                self.normalBackground = self._entryFieldEntry.cget(
                    "background")
                self._entryFieldEntry.configure(
...
def aligngrouptags(groups):
    ...
    for group in groups:
        if group._tag is None:
            height = string.atoi(str(
                group._ring.cget("borderwidth"))) + string.atoi(
                    str(group._ring.cget("highlightthickness")))
        else:
...
class Group(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._groupChildSite = self.createcomponent("groupchildsite", (), None,
                                                    tkinter.Frame,
                                                    (self._ring, ))
        ...
        ringBorder = string.atoi(str(
            self._ring.cget("borderwidth"))) + string.atoi(
                str(self._ring.cget("highlightthickness")))
        if self._tag is None:
            tagHeight = ringBorder
        else:
            tagHeight = self._tag.winfo_reqheight()
            self._tag.place(x=ringBorder + self["tagindent"],
                            y=tagHeight / 2,
                            anchor="w")
...
class LabeledWidget(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        # Create the components.
        interior = MegaWidget.interior(self)
        self._labelChildSite = self.createcomponent("labelchildsite", (), None,
                                                    tkinter.Frame,
                                                    (interior, ))
...
class MainMenuBar(MegaArchetype):
    ...
    def addmenu(self,
        ...
        if statusHelp is None:
            statusHelp = balloonHelp
        self._addmenu(None, menuName, balloonHelp, statusHelp, traverseSpec,
                      kw)
    ...
    def addcascademenu(self,
                       ...
                       traverseSpec=None,
                       **kw):
        self._addmenu(parentMenuName, menuName, None, statusHelp, traverseSpec,
                      kw)
    ...
    def _addmenu(self, parentMenuName, menuName, balloonHelp, statusHelp,
        ...
        menu = self.createcomponent(
            *(menuName, (), "Menu", tkinter.Menu, (parentMenu, )), **menukw)
        ...
        menu.bind(
            "<Motion>",
            lambda event=None, self=self, menuName=menuName: self._menuHelp(
                event, menuName),
        )
    ...
    def _addHotkeyToOptions(self, menuName, kw, traverseSpec):
        ...
        if not self["hotkeys"] or "underline" in kw or "label" not in kw:
            return
...
class MenuBar(MegaWidget):
    ...
    def _addmenu(
        ...
        menu = self.createcomponent(
            *(menuName + "-menu", (), "Menu", tkinter.Menu, (parentMenu, )),
            **menukw)
        if parentMenuName is None:
...
class MessageBar(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        # Create the components.
        interior = self.interior()
        self._messageBarEntry = self.createcomponent("entry", (), None,
                                                     tkinter.Entry,
                                                     (interior, ))
...
class MessageDialog(Dialog):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._message = self.createcomponent("message", (), None,
        ...
        if iconpos is None:
            ...
            self._icon = self.createcomponent("icon", (), None, tkinter.Label,
                                              (interior, ))
...
class NoteBook(MegaArchetype):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._lightBorderColor, self._darkBorderColor = Color.bordercolors(
            self, self["hull_background"])
        ...
        if self._withTabs:
            self._pageLeftBorder = self.create_polygon(
                0, 0, 0, 0, 0, 0, fill=self._lightBorderColor, tags="lighttag")
            self._pageBottomRightBorder = self.create_polygon(
                0, 0, 0, 0, 0, 0, fill=self._darkBorderColor, tags="darktag")
            self._pageTop1Border = self.create_polygon(
                0, 0, 0, 0, 0, 0, fill=self._darkBorderColor, tags="lighttag")
            self._pageTop2Border = self.create_polygon(
                0, 0, 0, 0, 0, 0, fill=self._darkBorderColor, tags="lighttag")
        else:
            self._pageLeftBorder = self.create_polygon(
                0, 0, 0, 0, 0, 0, fill=self._darkBorderColor, tags="darktag")
            self._pageBottomRightBorder = self.create_polygon(
                0, 0, 0, 0, 0, 0, fill=self._lightBorderColor, tags="lighttag")
            self._pageTopBorder = self.create_polygon(
                0, 0, 0, 0, 0, 0, fill=self._darkBorderColor, tags="darktag")
    ...
    def insert(self, pageName, before=0, **kw):
        ...
        page = self.createcomponent(
            *(pageName, (), "Page", tkinter.Frame, self._hull), **pageOptions)
        ...
        if self._withTabs:
            ...
            tab = self.createcomponent(
                *(pageName + "-tab", (), "Tab", tkinter.Button, self._hull),
                **tabOptions,
            )
            ...
            lightshadow = self.create_polygon(0,
                                              0,
                                              0,
                                              0,
                                              0,
                                              0,
                                              tags="lighttag",
                                              fill=self._lightBorderColor)
            darkshadow = self.create_polygon(0,
                                             0,
                                             0,
                                             0,
                                             0,
    ...
    def _layout(self):
        ...
        canvasBorder = string.atoi(self._hull["borderwidth"]) + string.atoi(
            self._hull["highlightthickness"])
        ...
        # Draw all the tabs.
        if self._withTabs and ("tabs" in self._pending
                               or "size" in self._pending):
            ...
            availableWidth = hullWidth - 2 * canvasBorder - numTabs * 2 * borderWidth
            x = canvasBorder
            cumTabReqWidth = 0
            cumTabWidth = 0
            ...
            for pageName in self._pageNames:
                ...
                if sumTabReqWidth <= availableWidth:
                    ...
                    tmp = (2 * cumTabReqWidth * availableWidth +
                           sumTabReqWidth) / (2 * sumTabReqWidth)
        ...
        if self._withTabs and ("topPage" in self._pending or "tabs"
                               in self._pending or "size" in self._pending):

            if self.getcurselection() is None:
                # No pages, so draw line across top of page area.
                self.coords(
                    self._pageTop1Border,
                    canvasBorder,
                    self.tabBottom,
                    hullWidth - canvasBorder,
                    self.tabBottom,
                ...
                self.coords(
                    ...
                    left + borderWidth,
                    self.tabBottom + borderWidth,
                    canvasBorder + borderWidth,
                    self.tabBottom + borderWidth,
                )

                self.coords(
                    self._pageTop2Border,
                    right,
                    self.tabBottom,
                    hullWidth - canvasBorder,
                    self.tabBottom,
        ...
        if "size" in self._pending or oldTabBottom != self.tabBottom:
            ...
            self.coords(
                ...
                canvasBorder,
                hullHeight - canvasBorder,
            )

            self.coords(
                self._pageBottomRightBorder,
                hullWidth - canvasBorder,
                self.tabBottom,
                hullWidth - canvasBorder,
                hullHeight - canvasBorder,
        ...
        if (newTopPage is not None
                or "size" in self._pending and self._topPageName is not None
...
class OptionMenu(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._menubutton = self.createcomponent(
            "menubutton",
            (),
            None,
            tkinter.Menubutton,
...
class PanedWidget(MegaWidget):
    ...
    def insert(self, name, before=0, **kw):
        ...
        self._frame[name] = self.createcomponent(name, (), "Frame",
                                                 tkinter.Frame,
                                                 (self.interior(), ))
        ...
        if atEnd:
            ...
            if size > 0 or self._relsize[name] is not None:
                if self["orient"] == "vertical":
                    self._frame[name].place(x=0,
                                            relwidth=1,
                                            height=size,
                                            y=self._totalSize)
                else:
                    self._frame[name].place(y=0,
                                            relheight=1,
                                            width=size,
                                            x=self._totalSize)
            else:
                if self["orient"] == "vertical":
                    self._frame[name].place(x=0, relwidth=1, y=self._totalSize)
                else:
                    self._frame[name].place(y=0,
                                            relheight=1,
                                            x=self._totalSize)
    ...
    def _plotHandles(self):
        ...
        for i in range(1, last + 1):
            ...
            if self["orient"] == "vertical":
                ...
                self._separator[i].place(x=0, y=totalSize, relwidth=1)
            else:
                ...
                self._separator[i].place(y=0, x=totalSize, relheight=1)
...
class PromptDialog(Dialog):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._promptDialogEntry = self.createcomponent("entryfield", aliases,
                                                       None, EntryField,
                                                       (interior, ))
        self._promptDialogEntry.pack(fill="x",
                                     expand=1,
                                     padx=self["borderx"],
                                     pady=self["bordery"])
...
class RadioSelect(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        if self["labelpos"] is None:
            ...
            self._radioSelectFrame = self.createcomponent(
                "frame", (), None, tkinter.Frame, (interior, ))
    ...
    def add(self, componentName, **kw):
        ...
        kw["command"] = lambda self=self, name=componentName: self.invoke(name)
        if "text" not in kw:
            kw["text"] = componentName
        ...
        button = self.createcomponent(
            *(componentName, (), "Button", self.buttonClass,
              (self._radioSelectFrame, )),
            **kw,
        )
...
class ScrolledCanvas(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        if self["borderframe"]:
            ...
            self._borderframe = self.createcomponent(
                "borderframe",
                (),
                None,
                tkinter.Frame,
        ...
        self._horizScrollbar = self.createcomponent(
            "horizscrollbar",
            (),
            "Scrollbar",
            tkinter.Scrollbar,
        ...
        self._vertScrollbar = self.createcomponent(
            "vertscrollbar",
            (),
            "Scrollbar",
            tkinter.Scrollbar,
    ...
    def _hscrollMode(self):
        ...
        if mode == "static":
            ...
            message = (
                'bad hscrollmode option "%s": should be static, dynamic, or none'
                % mode)
    ...
    def _vscrollMode(self):
        ...
        if mode == "static":
            ...
            message = (
                'bad vscrollmode option "%s": should be static, dynamic, or none'
                % mode)
            raise ValueError(message)
    ...
    def _configureScrollCommands(self):
        ...
        if self["hscrollmode"] == self["vscrollmode"] == "dynamic":
            ...
            self._canvas.configure(xscrollcommand=self._scrollXNow,
    ...
    def _scrollBothNow(self):
        ...
        if (self["hscrollmode"] == self["vscrollmode"] == "dynamic"
                and self._horizScrollbarNeeded != self._horizScrollbarOn
                and self._vertScrollbarNeeded != self._vertScrollbarOn
                and self._vertScrollbarOn != self._horizScrollbarOn):
            if self._horizScrollbarOn:
                self._toggleHorizScrollbar()
            else:
                self._toggleVertScrollbar()
    ...
    def _setRegion(self):
        ...
        if region is not None:
            ...
            region = (
                region[0] - canvasmargin,
                region[1] - canvasmargin,
                region[2] + canvasmargin,
                region[3] + canvasmargin,
            )
...
class ScrolledField(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        # Create the components.
        interior = self.interior()
        self._scrolledFieldEntry = self.createcomponent(
...
class ScrolledFrame(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        if self["borderframe"]:
            ...
            self._clipper = self.createcomponent(
                "clipper",
                (),
                None,
                tkinter.Frame,
        ...
        # scrolled.
        self._frame = self.createcomponent("frame", (), None, tkinter.Frame,
                                           (self._clipper, ))
        ...
        if frameWidth <= clipperWidth:
            ...
            if self["horizflex"] in ("shrink", "elastic"):
                ...
                if self.startX + clipperWidth > frameWidth:
                    ...
                    if self.startX < 0:
                        self.startX = 0
                    endScrollX = (self.startX +
                                  clipperWidth) / float(frameWidth)
                relwidth = ""
        ...
        if frameHeight <= clipperHeight:
            ...
            if self["vertflex"] in ("shrink", "elastic"):
                ...
                if self.startY + clipperHeight > frameHeight:
                    ...
                    if self.startY < 0:
                        self.startY = 0
                    endScrollY = (self.startY +
                                  clipperHeight) / float(frameHeight)
                relheight = ""
...
class ScrolledListBox(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._listbox = self.createcomponent("listbox", (), None,
    ...
    def _configureScrollCommands(self):
        ...
        if self["hscrollmode"] == self["vscrollmode"] == "dynamic":
            ...
            self._listbox.configure(xscrollcommand=self._scrollXNow,
                                    yscrollcommand=self._scrollYNow)
    ...
    def _handleEvent(self, event, eventType):
        if eventType == "double":
            ...
            # outside the listbox.
            if (event.x < 0 or self._listbox.winfo_width() <= event.x
                    or event.y < 0 or self._listbox.winfo_height() <= event.y):
                return
...
class ScrolledText(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        if self["borderframe"]:
            ...
            self._textbox = self.createcomponent(
                "text",
                (),
                None,
                tkinter.Text,
        ...
        if self["columnheader"]:
            self._columnheader = self.createcomponent(
                "columnheader",
                (),
                "Header",
                tkinter.Text,
        ...
        if self["rowheader"]:
            self._rowheader = self.createcomponent(
                "rowheader",
                (),
                "Header",
                tkinter.Text,
            ...
            self._rowheader.configure(yscrollcommand=self._rowheaderscrolled)

        if self["rowcolumnheader"]:
            self._rowcolumnheader = self.createcomponent(
                "rowcolumnheader",
                (),
                "Header",
                tkinter.Text,
    ...
    def _configureScrollCommands(self):
        ...
        if self["hscrollmode"] == self["vscrollmode"] == "dynamic":
            ...
            self._textbox.configure(xscrollcommand=self._scrollXNow,
                                    yscrollcommand=self._scrollYNow)
    ...
    def _scrollXNow(self, first, last):
        ...
        if self["hscrollmode"] == "dynamic":
            ...
            if self._horizScrollbarNeeded != self._horizScrollbarOn:
                if self._horizScrollbarNeeded or self._textWidth != currentWidth:
                    self._toggleHorizScrollbar()
...
class SelectionDialog(Dialog):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._list = self.createcomponent(
            "scrolledlist",
            aliases,
            None,
            ScrolledListBox,
...
class TextDialog(Dialog):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._text = self.createcomponent("scrolledtext", aliases, None,
...
class TimeCounter(MegaWidget):
    ...
    def _createComponents(self, kw):
        ...
        self._downHourArrowBtn = self.createcomponent(
            "downhourarrow",
            (),
            "Arrow",
            tkinter.Canvas,
        ...
        self._downMinuteArrowBtn = self.createcomponent(
            "downminutearrow",
            (),
            "Arrow",
            tkinter.Canvas,
        ...
        self._downSecondArrowBtn = self.createcomponent(
            "downsecondarrow",
            (),
            "Arrow",
            tkinter.Canvas,
        ...
        self._hourCounterEntry = self.createcomponent(
            "hourentryfield",
            (("hourentry", "hourentryfield_entry"), ),
            None,
            EntryField,
            (frame, ),
            validate="integer",
        ...
        self._minuteCounterEntry = self.createcomponent(
            "minuteentryfield",
            (("minuteentry", "minuteentryfield_entry"), ),
            None,
            EntryField,
            (frame, ),
            validate="integer",
        ...
        self._secondCounterEntry = self.createcomponent(
            "secondentryfield",
            (("secondentry", "secondentryfield_entry"), ),
            None,
            EntryField,
            (frame, ),
            validate="integer",
        ...
        self._upHourArrowBtn = self.createcomponent(
            "uphourarrow",
            (),
            "Arrow",
            tkinter.Canvas,
        ...
        self._upMinuteArrowBtn = self.createcomponent(
            "upminutearrow",
            (),
            "Arrow",
            tkinter.Canvas,
        ...
        self._upSecondArrowBtn = self.createcomponent(
            "upsecondarrow",
            (),
            "Arrow",
            tkinter.Canvas,
        ...
        self._upHourArrowBtn.bind(
            "<Configure>",
            lambda event, s=self, button=self._upHourArrowBtn: s._drawArrow(
                button, "up"),
        )

        self._upHourArrowBtn.bind(
            "<1>",
            lambda event, s=self, button=self._upHourArrowBtn: s._countUp(
                button, 3600),
        )

        self._upHourArrowBtn.bind(
            "<Any-ButtonRelease-1>",
            lambda event, s=self, button=self._upHourArrowBtn: s._stopUpDown(
                button),
        )
        ...
        self._upMinuteArrowBtn.bind(
            "<Configure>",
            lambda event, s=self, button=self._upMinuteArrowBtn: s._drawArrow(
                button, "up"),
        )

        self._upMinuteArrowBtn.bind(
            "<1>",
            lambda event, s=self, button=self._upMinuteArrowBtn: s._countUp(
                button, 60),
        )

        self._upMinuteArrowBtn.bind(
            "<Any-ButtonRelease-1>",
            lambda event, s=self, button=self._upMinuteArrowBtn: s._stopUpDown(
                button),
        )

        # Up second
        self._upSecondArrowBtn.bind(
            "<Configure>",
            lambda event, s=self, button=self._upSecondArrowBtn: s._drawArrow(
                button, "up"),
        )

        self._upSecondArrowBtn.bind(
            "<1>",
            lambda event, s=self, button=self._upSecondArrowBtn: s._countUp(
                button, 1),
        )

        self._upSecondArrowBtn.bind(
            "<Any-ButtonRelease-1>",
            lambda event, s=self, button=self._upSecondArrowBtn: s._stopUpDown(
                button),
        )
        ...
        self._downHourArrowBtn.bind(
            "<Configure>",
            lambda event, s=self, button=self._downHourArrowBtn: s._drawArrow(
                button, "down"),
        )

        self._downHourArrowBtn.bind(
            "<1>",
            lambda event, s=self, button=self._downHourArrowBtn: s._countDown(
                button, 3600),
        )
        self._downHourArrowBtn.bind(
            "<Any-ButtonRelease-1>",
            lambda event, s=self, button=self._downHourArrowBtn: s._stopUpDown(
                button),
        )
        ...
        self._downMinuteArrowBtn.bind(
            "<Configure>",
            lambda event, s=self, button=self._downMinuteArrowBtn: s.
            _drawArrow(button, "down"),
        )

        self._downMinuteArrowBtn.bind(
            "<1>",
            lambda event, s=self, button=self._downMinuteArrowBtn: s.
            _countDown(button, 60),
        )
        self._downMinuteArrowBtn.bind(
            "<Any-ButtonRelease-1>",
            lambda event, s=self, button=self._downMinuteArrowBtn: s.
        ...
        # Down second
        self._downSecondArrowBtn.bind(
            "<Configure>",
            lambda event, s=self, button=self._downSecondArrowBtn: s.
            _drawArrow(button, "down"),
        )
        ...
        self._hourCounterEntry.component("entry").bind("<Return>",
                                                       self._invoke)
    ...
    def _resizeArrow(self, event=None):
        for btn in (
                self._upHourArrowBtn,
                self._upMinuteArrowBtn,
                self._upSecondArrowBtn,
                self._downHourArrowBtn,
                self._downMinuteArrowBtn,
                self._downSecondArrowBtn,
        ):
            bw = string.atoi(btn["borderwidth"]) + string.atoi(
                btn["highlightthickness"])
        ...
        if newFlag != "force":
            if self["autorepeat"]:
                ...
                self._timerId = self.after(
                    delay,
                    lambda self=self, factor=factor, increment=increment: self.
                    _count(factor, "running", increment),
                )
...
class ComboBox(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._entryfield = self.createcomponent(
            "entryfield",
            (("entry", "entryfield_entry"), ),
            None,
            EntryField,
        ...
        if self["dropdown"]:
            ...
            self._popup = self.createcomponent("popup", (), None,
    ...
    def _dropdownBtnRelease(self, event):
        if event.widget == self._list.component(
                "vertscrollbar") or event.widget == self._list.component(
                    "horizscrollbar"):
            return
        ...
        if (event.x >= 0 and event.x < self.__listbox.winfo_width()
                and event.y >= 0 and event.y < self.__listbox.winfo_height()):
            self._selectCmd()
    ...
    def _resizeArrow(self, event):
        bw = string.atoi(self._arrowBtn["borderwidth"]) + string.atoi(
            self._arrowBtn["highlightthickness"])
...
class ComboBoxDialog(Dialog):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._combobox = self.createcomponent(
            "combobox",
            aliases,
            None,
            ComboBox,
...
class Counter(MegaWidget):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._downArrowBtn = self.createcomponent(
            "downarrow",
            (),
            "Arrow",
            tkinter.Canvas,
        ...
        self._counterEntry = self.createcomponent(
            "entryfield", (("entry", "entryfield_entry"), ), None, EntryField,
            (frame, ))

        # Create the up arrow.
        self._upArrowBtn = self.createcomponent(
            "uparrow",
            (),
            "Arrow",
            tkinter.Canvas,
        ...
        if orient == "horizontal":
            ...
            self._counterEntry.grid(column=1, row=0, sticky=self["sticky"])
            ...
            self._counterEntry.grid(column=0, row=1, sticky=self["sticky"])
    ...
    def _countUp(self, event):
        self._normalRelief = (self._upArrowBtn,
                              self._upArrowBtn.cget("relief"))
    ...
    def _countDown(self, event):
        self._normalRelief = (self._downArrowBtn,
                              self._downArrowBtn.cget("relief"))
    ...
    def _datatype(self):
        ...
        if datatype in _counterCommands:
            ...
            validValues.sort()
            raise ValueError(
                ('bad datatype value "%s":  must be a' +
    ...
    def _forceCount(self, factor):
        ...
        try:
            value = self._counterCommand(*(text, factor, self["increment"]),
                                         **self._counterArgs)
        except ValueError:
            self.bell()
            return
    ...
    def _count(self, factor, first):
        ...
        if self["autorepeat"]:
            if first:
                ...
                delay = self["repeatrate"]
            self._timerId = self.after(
                delay, lambda self=self, factor=factor: self._count(factor, 0))
...
def _changeDate(value, factor, increment, format="ymd", separator="/", yyyy=0):
...
def _changeTime(value, factor, increment, separator=":", time24=0):
    ...
    mins = unixTime % 60
    hours = unixTime / 60
    return "%s%02d%s%02d%s%02d" % (sign, hours, separator, mins, separator,
                                   secs)
...
class CounterDialog(Dialog):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self._cdCounter = self.createcomponent("counter", aliases, None,
                                               Counter, (interior, ))
        self._cdCounter.pack(fill="x",
                             expand=1,
                             padx=self["borderx"],
                             pady=self["bordery"])
