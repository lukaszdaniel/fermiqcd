...
mdp_real add_const(mdp_real x)
{
  return x + 5;
}

mdp_real add_two(mdp_real a, mdp_real b)
{
  return a + b;
}
...
int main()
{
  ...
  mdp_array<mdp_real, 3> a(3, 3, 3);
  for (mdp_suint i = 0; i < 3; i++)
    for (mdp_suint j = 0; j < 3; j++)
      for (mdp_suint k = 0; k < 3; k++)
  ...
  mdp_array<mdp_real, 3> b = a;
  std::cout << "Array b (copy of a):\n"
            << b << std::endl;

  // 4. Przypisanie
  mdp_array<mdp_real, 3> c(3, 3, 3);
  c = a;
  std::cout << "Array c (assigned from a):\n"
            << c << std::endl;

  // 5. operator +
  mdp_array<mdp_real, 3> d = a + b;
  std::cout << "a + b:\n"
            << d << std::endl;

  // 6. operator -
  mdp_array<mdp_real, 3> e = d - a;
  std::cout << "d - a:\n"
            << e << std::endl;

  // 7. operator * (skalar)
  mdp_array<mdp_real, 3> f = 2.0f * a;
  std::cout << "2 * a:\n"
            << f << std::endl;

  // 8. applytoall (unarny)
  mdp_array<mdp_real, 3> g = applytoall(a, add_const);
  std::cout << "a + 5 (applytoall):\n"
            << g << std::endl;

  // 9. applytoall (binarny)
  mdp_array<mdp_real, 3> h = applytoall(a, b, add_two);
  ...
  // 13. Prosty assert
  mdp_array<mdp_suint, 1> test(5);
  for (mdp_suint i = 0; i < 5; i++)
    test[i] = i;
