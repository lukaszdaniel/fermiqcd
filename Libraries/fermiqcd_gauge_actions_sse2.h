...
private:
  ...
  static int strange_mapping(site &x)
  {
    ...
    for (mu = 0; mu < x.lattice().ndim; mu++)
      type += (int)pow((float)iGauge_min, mu) * (x(mu) % iGauge_min);
    return type;
  }
...
public:
                              ...
                              std::string model = "MILC")
  {
    ...
    if (coeff.has_key("zeta"))
      ...
      zeta = 1;
    if (coeff.has_key("u_t"))
      u_t = coeff["u_t"];
    else
      u_t = 1;
    if (coeff.has_key("u_s"))
      u_s = coeff["u_s"];
    else
      u_s = 1;
              ...
              for (i = 0; i < nc - 1; i++)
                for (j = i + 1; j < nc; j++)
                {
                  if (zeta != 1)
                  {
                    ...
                    if (mu == 0)
                      M = U(x, mu) * (c_tp * staple_0i_H(U, x, 0) +
                                      c_tr * rectangles_0i_H(U, x, 0));
                    else
                      M = U(x, mu) * (c_sp * staple_ij_H(U, x, mu) +
