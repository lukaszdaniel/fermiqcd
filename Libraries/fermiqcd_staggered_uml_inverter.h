...
private:
	static inversion_stats staggered_BiCG_QQh(staggered_field &psi_out,
			staggered_field &psi_in, gauge_field &U, coefficients &coeff,
			mdp_real mass, int parity = EVEN, mdp_real absolute_precision =
					staggered_inversion_precision, mdp_real relative_precision =
					0, int max_steps = 2000) {
		...
		do {
			...
			if (rresidue < relative_precision)
				break;
			...
			if ((ME == 0) && (step > 100) && (residue == old_residue))
				error(
						"fermiqcd_staggered_uml_inverter/staggered_BiCG_QQh: not converging");
			step++;
		} while (step < max_steps);
		mdp << "UML BiCG converged in " << step << " iterations and "
				<< mpi.time() - time << " seconds\n",
...
public:
	static inversion_stats inverter(staggered_field &psi_out,
		...
		// It is important to initialize the output here
		...
		stats = staggered_BiCG_QQh(psi_out, r, U, coeff, mass, EVEN,
