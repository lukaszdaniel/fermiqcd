...
void mdp_field<T>::update(int np, int d, int ncomp) {
	...
	for (dp = 1; dp < Nproc; dp++) {
		...
		if (np < 2) {
			...
			length = lattice().len_to_send[process][0]
					+ lattice().len_to_send[process][1];
		}
		if (np == 1)
		...
		if (length > 0) {
			...
			for (idx = 0; idx < length; idx++)
				for (k = 0; k < ncomp; k++)
					dynamic_buffer[idx * ncomp + k] = *(m
							+ lattice().to_send[process][start_to_send + idx]
		...
		if (length > 0) {
			if (ncomp == field_components) {
				...
				for (idx = 0; idx < length; idx++)
					for (k = 0; k < ncomp; k++)
						*(m
								+ (lattice().start[process][ni] + idx)
										* field_components + d * ncomp + k) =
								where_to[idx * ncomp + k];
				delete[] where_to;
			}
		}
