...
mdp_real TimePlaquette(gauge_field &U) {
	...
	for (int nu = mu + 1; nu < U.ndim; nu++)
		tmp += real(trace(plaquette(U, x, mu, nu)));
	mdp.add(tmp);
	return tmp / (U.lattice().nvol_gl * (U.ndim - 1));
}
...
mdp_real SpacePlaquette(gauge_field &U) {
	...
	forallsites (x)
	for (int mu = 1; mu < U.ndim - 1; mu++)
		for (int nu = mu + 1; nu < U.ndim; nu++)
			tmp += real(trace(plaquette(U, x, mu, nu)));
	mdp.add(tmp);
	return 2 * tmp / (U.lattice().nvol_gl * (U.ndim - 2) * (U.ndim - 1));
}
...
mdp_matrix_field PolyakovField(gauge_field &U) {
	...
	forallsites(x)
	{
		if (x(0) == 0) {
			y = x;
			L(x) = U(x, 0);
			for (unsigned short int i = 1; i < U.lattice().size(0); i++) {
				y = y + 0;
				L(x) *= U(y, 0);
			}
		}
...
mdp_complex PolyakovLoop(gauge_field &U) {
	...
	return sum * U.lattice().size(0) / (U.lattice().nvol_gl * U.nc);
}

/// Relaxation algorithm
static void relaxation(gauge_field &U, int n_iter = 1) {
	if (U.nc == 1)
					...
					for (int i = 0; i < U.nc - 1; i++)
						for (int j = i + 1; j < U.nc; j++) {
							...
							for (int k = 0; k < U.nc; k++) {
								tmpUik = (a[0] * a[0] + a[2] * a[1])
										* U(x, mu, i, k)
										+ (a[1] * (a[0] + a[3]))
												* U(x, mu, j, k);
								U(x, mu, j, k) = (a[2] * (a[0] + a[3]))
										* U(x, mu, i, k)
										+ (a[1] * a[2] + a[3] * a[3])
												* U(x, mu, j, k);
								U(x, mu, i, k) = tmpUik;
...
mdp_real unitarize(gauge_field &U) {
	...
	forallsites(x)
	{
		for (int mu = 0; mu < U.ndim; mu++) {
			...
			for (int i = 0; i < U.nc; i++) {
				...
				for (int j = 0; j < U.nc; j++)
					e(i, j) /= sqrt(quadnorm);	//ei = ei/||ei||
			}

			//The last vector is calculated as the cross product: ek = ei x ej
			i = U.nc - 1;
			for (int j = 0; j < U.nc; j++) {
				for (int s = 0; s < U.nc - 1; s++) {
					for (int k = 0; k < U.nc - 1; k++) {
						if (k < j)
							m(s, k) = e(s, k);
						else
							m(s, k) = e(s, k + 1);
	...
	mpi.add(precision);
	stats = precision / (U.ndim * U.lattice().nvol_gl);
...
mdp_matrix PolyCor(gauge_field &U) {
	...
	for (int mu = 1; mu < U.ndim; mu++) {
		for (int i = 0; i < U.lattice().size(1); i++) {
			proj(mu - 1, i) = 0.0;
			for (int k = 0; k < U.lattice().size(1); k++) {
				if (mu == 1)
					x.set(0, i, k);
				else
					x.set(0, k, i);
...
void compute_em_field(gauge_field &U) {
	...
	for (mu = 0; mu < U.ndim - 1; mu++)
		for (nu = mu + 1; nu < U.ndim; nu++) {
			...
			A = U(x, mu) * U(x + mu, nu) * hermitian(U(x, nu) * U(x + nu, mu))
					+ hermitian(U(x - nu, nu)) * U(x - nu, mu)
							* U((x - nu) + mu, nu) * hermitian(U(x, mu))
					+ hermitian(U((x - mu) - nu, nu) * U(x - mu, mu))
							* U((x - mu) - nu, mu) * U(x - nu, nu)
					+ U(x, nu) * hermitian(U(x - mu, nu) * U((x + nu) - mu, mu))
							* U(x - mu, mu);
...
void compute_long_links(gauge_field &U, gauge_field &V, int length = 2) {
	if ((&(U.lattice()) != &(V.lattice())) || (U.nc != V.nc)
			|| (U.ndim != V.ndim))
		error(
				"fermiqcd_gauge_auxiliary_functions/compute_long_links: U and V are not compatible lattices");
	if (V.lattice().next_next < length)
		error(
				"fermiqcd_gauge_auxiliary_functions/compute_long_links: next_next is not big enough");
