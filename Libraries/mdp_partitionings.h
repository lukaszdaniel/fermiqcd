...
namespace MDP
{
   ...
   */
  template <mdp_suint dim>
  mdp_uint default_partitioning(const mdp_uint x[],
                          [[maybe_unused]] const mdp_suint ndim,
                          const mdp_uint nx[])
  {
    mdp_uint partition_factor = std::ceil((1.0 * nx[dim]) / Nproc);
   ...
   * function
   */
  mdp_uint default_partitioning0(const mdp_uint x[],
