...
public:
	...
	float rho;
	int charge; // +1 or -1;
	SingleInstanton4D(float x0, float x1, float x2, float x3, float rho,
			int charge) {
		x[0] = x0;
		x[1] = x1;
		x[2] = x2;
...
private:
	...
	void init_tau_and_bar() {
		tau[0] = sigma[0]; // Identity
		tau[1] = I * sigma[1];
		tau[2] = I * sigma[2];
		tau[3] = I * sigma[3];
		bar[0] = sigma[0]; // Identity
		bar[1] = -I * sigma[1];
		bar[2] = -I * sigma[2];
		bar[3] = -I * sigma[3];
	}

	Matrix make_singular_instanton(float xl[4], int mu,
			SingleInstanton4D &instanton) {
		Matrix A;
		float x2, x[4], rho2;
		for (int i = 0; i < 4; i++)
		...
		if (instanton.charge > 0)
			A = (-0.5 * rho2 / (x2 + rho2) / x2)
					* (bar[mu]
							* (x[0] * tau[0] + x[1] * tau[1] + x[2] * tau[2]
									+ x[3] * tau[3])
							- (x[0] * bar[0] + x[1] * bar[1] + x[2] * bar[2]
									+ x[3] * bar[3]) * tau[mu]);
		else
			A = (-0.5 * rho2 / (x2 + rho2) / x2)
					* (tau[mu]
	...
	Matrix make_su2_link(mdp_site xn, int mu,
			vector<SingleInstanton4D> &instantons) {
		Matrix A(2, 2);
		Matrix P = sigma[0];
