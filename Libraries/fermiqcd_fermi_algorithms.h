...
void (*default_fermi_action)(fermi_field &, fermi_field &, gauge_field &,
		coefficients &, int) = FermiCloverActionFast::mul_Q;

/// Calls the current Wilson/Clover action
void mul_Q(fermi_field &psi_out, fermi_field &psi_in, gauge_field &U,
...
inversion_stats (*default_fermi_inverter)(fermi_field &, fermi_field &,
		gauge_field &, coefficients &, mdp_real, mdp_real,
		int)=&(MinRes::inverter<fermi_field,gauge_field>);

/// Executes the current Wilson/Clover inverter
inversion_stats mul_invQ(fermi_field &psi_out, fermi_field &psi_in,
		gauge_field &U, coefficients &coeff, mdp_real absolute_precision =
				fermi_inversion_precision, mdp_real relative_precision = 0,
		int max_steps = 2000) {
	return (*default_fermi_inverter)(psi_out, psi_in, U, coeff,
			absolute_precision, relative_precision, max_steps);
}

/// Checks that inversion is working
mdp_real check_inversion(fermi_field &phi, gauge_field &U,
		coefficients &coeff) {
