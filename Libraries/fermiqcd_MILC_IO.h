...
bool milc_read_as_float_noswitch(FILE *fp, void* data, mdp_int psize,
	...
	position = (((x(0) * lattice.size(3) + x(3)) * lattice.size(2) + x(2))
			* lattice.size(1) + x(1));
...
bool milc_read_as_float_switch(FILE *fp, void* data, mdp_int psize,
		mdp_int header_size, mdp_int position, const mdp_lattice &lattice) {
...
bool load_milc(gauge_field &U, string filename, mdp_int max_buffer_size = 128,
		int processIO = 0) {
	...
	if (U.lattice().ndim != 4 || milc_header.dims[0] != U.lattice().size(1)
