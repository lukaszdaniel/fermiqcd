...
public:
	...
	mdp_real lambda;
	mdp_matrix eta[4][4];
	Instanton4D(int nc, int sub_i, int sub_j, mdp_real charge, mdp_real lambda,
			vector<mdp_real> &p) {
		this->nc = nc;
		this->sub_i = sub_i;
		this->sub_j = sub_j;
		...
		sigma_rot3[2] = -sin(gamma) * sigma_rot2[1]
				+ cos(gamma) * sigma_rot2[2];
