...
/// Auxiliary function
bool mdp_default_user_read(FILE *fp, void* p, mdp_int psize,
		mdp_int header_size, mdp_int position, const mdp_lattice &lattice) {
...
template<class T>
bool mdp_field<T>::load(string filename, int processIO, mdp_int max_buffer_size,
		bool load_header, mdp_int skip_bytes,
		bool (*user_read)(FILE*, void*, mdp_int, mdp_int, mdp_int,
				const mdp_lattice&), bool try_swicth_endianess) {
	...
	if (ME == processIO) {
		...
		cout << "Loading file " << filename << " from process " << processIO
		...
		if (load_header) {
			...
			if (fseek(fp, skip_bytes, SEEK_SET)
				...
				fprintf(stderr,
						"mdp_field.load(): Unable to load file header\n");
				return false;
			}
			...
			if (tmp_header.endianess != header.endianess)
				fprintf(stderr,
						"Unrecognized endianess... trying to read anyway\n");
			...
			header_size += total_size
					- (tmp_header.bytes_per_site * actual_size + header_size);
			...
			if (tmp_header.bytes_per_site != header.bytes_per_site) {
				fprintf(stderr,
						"mdp_file.load(): wrong type of field (%i bytes per site?)\n",
		...
		for (idx_gl = 0; idx_gl < nvol_gl; idx_gl++) {
			...
			if (process != NOWHERE) {
				if (user_read) {
					if (!user_read(fp, short_buffer, field_components * Tsize,
							skip_bytes, idx_gl, lattice()))
						error("unexpected end of file");
				} else {
					if (exception
							&& fseek(fp, idx_gl * psize + skip_bytes,
									SEEK_SET)) {
						cout << "debug info: " << idx_gl * psize + skip_bytes
								<< " " << psize << endl;
						error("unexpected end of file");
					}
			...
			if (process == processIO) {
				for (k = 0; k < field_components; k++)
					*(m + lattice().local(idx_gl) * field_components + k) =
							short_buffer[k];
			}
		}
		delete[] buffer_size;
