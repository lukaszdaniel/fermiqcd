...
namespace MDP
{
	...
	public:
		...
		{
			float err2;
			err2 = num * (std::pow((double)error, (double)2.0) + mean * mean) + std::pow((double)x, (double)2.0);
			num++;
		...
		friend mdp_measure operator/(mdp_measure a, mdp_measure b)
		{
			...
			tmp.error = a.error / fabs(b.mean) +
						fabs(a.mean) / std::pow((double)b.mean, (double)2.0) * b.error;
			tmp.num = 1;
			return tmp;
		}
		...
		friend mdp_measure operator/(float a, mdp_measure b)
		{
			mdp_measure tmp;
			tmp.mean = a / b.mean;
			tmp.error = fabs(a) / std::pow((double)b.mean, (double)2.0) * b.error;
			tmp.num = 1;
			return tmp;
		}
		...
		friend mdp_measure sin(mdp_measure a)
		{
			...
			tmp.error = fabs(std::cos(a.mean)) * a.error;
			tmp.num = 1;
			return tmp;
		}
		...
		friend mdp_measure cos(mdp_measure a)
		{
			...
			tmp.error = fabs(std::sin(a.mean)) * a.error;
			tmp.num = 1;
			return tmp;
		}
