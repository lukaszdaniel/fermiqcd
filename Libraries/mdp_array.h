...
public:
	...
	inline void dimension(const uint c0_ = 1, const uint c1_ = 1,
			const uint c2_ = 1, const uint c3_ = 1, const uint c4_ = 1) {
		if (m != 0)
			delete[] m;
		c[0] = c0_;
		imax = c0_;
	...
	inline mdp_array(const uint c0_ = 1, const uint c1_ = 1, const uint c2_ = 1,
			const uint c3_ = 1, const uint c4_ = 1) {
		flag = FREE;
		nc = nc_;
		m = 0;
	...
	inline mdp_array(const T *m0, const uint c0_ = 1, const uint c1_ = 1,
			const uint c2_ = 1, const uint c3_ = 1, const uint c4_ = 1) {
		flag = HARD;
		nc = nc_;
		m = 0;
	...
	inline mdp_array(const T *m0, const uint* p) {
		flag = HARD;
		nc = nc_;
		m = m0;
		imax = c[0] = p[0];
	...
	inline friend mdp_array applytoall(const mdp_array& a, T (*fptr)(T, void*),
			void *x = 0) {
		mdp_array tmp(a.c);
		for (register uint i = 0; i < a.imax; i++)
			tmp[i] = (*fptr)(a[i], x);
