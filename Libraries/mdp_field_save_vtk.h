 ...
 */
template<class T>
bool mdp_field<T>::save_vtk(string filename, int t, int component,
		int processIO, bool ASCII) {
	...
	if (ME == processIO) {
		...
		cout << "Saving file " << filename << " from process " << processIO
		...
		sprintf(header, "# vtk DataFile Version 2.0\n"
				...
				"ORIGIN     0   0   0\n"
				"SPACING    1   1   1\n"
				"POINT_DATA %i", filename.c_str(),
				((ASCII) ? "ASCII" : "BINARY"), LX, LY, LZ, LX * LY * LZ);
		...
		for (idx_gl = 0; idx_gl < nvol_gl; idx_gl++) {
			...
			if (process == processIO) {
				for (k = 0; k < field_components; k++)
					short_buffer[k] = *(m
							+ lattice().local(idx_gl) * field_components + k);
			}
			if (process != NOWHERE) {
...
mdp_field<float>& cumulate_field(mdp_field<float>& field, string filename) {
	...
	if (counter.find(filename) == counter.end()) {
		fields[filename] = new mdp_field<float>(field.lattice(),
				field.size_per_site());
		forallsites (p)
		...
		forallsites (p)
		for (int i = 0; i < field.size_per_site(); i++)
			(*fields[filename])(p, i) = (k * (*fields[filename])(p, i)
					+ field(p, i)) / (k + 1);
		counter[filename]++;
	}
...
/*
void save_vtk(mdp_field<float> &field, string filename, int t = -1,
		int component = -1, int processIO = 0, bool ASCII = false) {
	static map<string, int> counter;
	static map<string, mdp_field<float>*> fields;
	mdp_site p(field.lattice());
	int k = 0;
	...
	if (filename[filename.size() - 1] != '*') {
		...
		fields[filename]->save_vtk(filename + "_average.vtk", t, component,
				processIO, ASCII);
	} else {
		string filename2 = filename.substr(0, filename.size() - 1);
		field.save_vtk(filename2 + "_" + tostring(k) + ".vtk", t, component,
				processIO, ASCII);
		fields[filename]->save_vtk(
				filename2 + "_average_" + tostring(k) + ".vtk", t, component,
				processIO, ASCII);
	}
}
*/
