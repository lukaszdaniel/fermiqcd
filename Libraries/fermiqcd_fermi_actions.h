...
public:
	static void mul_Q(fermi_field &psi_out, fermi_field &psi_in, gauge_field &U,
		...
		if (psi_in.nc != U.nc)
			error(
					"fermiqcd_fermi_algorithms/mul_Q_ONE: gauge and spinor have different nc");
		if (parity != EVENODD)
		...
		if (kappa_t == 0)
			error("kappa_t=0 or undeclared");
		if (kappa_s == 0)
			error("kappa_s=0 or undeclared");
		...
		forallsites(x)
		{
			for (mu = 0; mu < ndim; mu++) {
				...
				if (mu == 0)
					psi_out(x) -= (kappa_t)
							* ((r_t - Gamma[mu] * rsign) * psi_up
									+ (r_t + Gamma[mu] * rsign) * psi_dw);
				else
					psi_out(x) -= (kappa_s)
							* ((r_s - Gamma[mu] * rsign) * psi_up
			...
			if (cSW != 0)
				for (mu = 0; mu < ndim - 1; mu++)
					for (nu = mu + 1; nu < ndim; nu++) {
						...
						if (mu == 0) {
							psi_out(x) += (kappa_s * cSW * c_E * I)
									* (Sigma[mu][nu] * psi_lo);
						} else {
							psi_out(x) += (kappa_s * cSW * c_B * I)
									* (Sigma[mu][nu] * psi_lo);
						}
					}
		}
...
public:
	static void mul_Q(fermi_field &psi_out, fermi_field &psi_in, gauge_field &U,
		...
		if (psi_in.nc != U.nc)
			error(
					"fermiqcd_fermi_algorithms/mul_Q_TWO: gauge and spinor have different nc");
		if (parity != EVENODD)
			error("parity must be EVENODD here");
				...
				for (mu = 0; mu < ndim - 1; mu++)
					for (nu = mu + 1; nu < ndim; nu++)
						for (a = 0; a < nspin; a++) {
							...
							if (mu == 0)
								coeff_clover = (kappa_s * cSW * c_E * I)
										* Sigma_val[mu][nu][a];
							else
								coeff_clover = (kappa_s * cSW * c_B * I)
										* Sigma_val[mu][nu][a];
							for (i = 0; i < nc; i++) {
								inc = i * nc;
