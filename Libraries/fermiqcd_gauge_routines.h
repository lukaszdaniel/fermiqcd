...
// ///////////////////////////
inline mdp_matrix staple(gauge_field &U, register site x, int mu, int s1,
		int nu) {
...
inline mdp_matrix staple_H(gauge_field &U, register site x, int mu, int s1,
		int nu) {
	mdp_matrix tmp(U.nc, U.nc);
	if (s1 == +1) {
...
inline mdp_matrix staple_H(gauge_field &U, register site x, int mu) {
	...
	for (nu = 0; nu < U.ndim; nu++)
		if (nu != mu) {
			tmp += U(x + mu, nu) * hermitian(U(x + nu, mu))
					* hermitian(U(x, nu));
			y = x - nu;
			tmp += hermitian(U(y + mu, nu)) * hermitian(U(y, mu)) * U(y, nu);
		}
...
inline mdp_matrix staple_H_unisotropic(gauge_field &U, register site x, int mu,
		mdp_real zeta) {
	mdp_matrix tmp(U.nc, U.nc);
	site y(U.lattice());
	int nu;
	...
	for (nu = 0; nu < U.ndim; nu++)
		if (nu != mu) {
			...
			tmp += param * U(x + mu, nu) * hermitian(U(x + nu, mu))
					* hermitian(U(x, nu));
			y = x - nu;
			tmp += param * hermitian(U(y + mu, nu)) * hermitian(U(y, mu))
					* U(y, nu);
		}
	return tmp;
}
		...
		for (nu = 1; nu < U.ndim; nu++)
			if (nu != mu) {
				tmp += U(x + mu, nu) * hermitian(U(x + nu, mu))
						* hermitian(U(x, nu));
				y = x - nu;
				tmp += hermitian(U(y + mu, nu)) * hermitian(U(y, mu))
						* U(y, nu);
			}
	return tmp;
}
...
inline mdp_matrix plaquette(gauge_field &U, site x, int mu, int nu) {
	mdp_matrix tmp(U.nc, U.nc);
	tmp = U(x, mu) * U(x + mu, nu) * hermitian(U(x + nu, mu))
			* hermitian(U(x, nu));
	return tmp;
}
...
/* obsolete stuff 
 ...
 void fast_mul_AB_to_C(mdp_complex *a, mdp_complex *b, mdp_complex *c, int &ni) {
...
void fast_add_AB_to_C(mdp_complex *a, mdp_complex *b, mdp_complex *c, int &ni) {
	for (register int i = 0; i < ni * ni; i++)
		c[i] = a[i] + b[i];
}
...
void fast_add_aAbB_to_C(mdp_complex c1, mdp_complex *a1, mdp_complex c2,
		mdp_complex *a2, mdp_complex *c, int &ni) {
	for (register int i = 0; i < ni * ni; i++)
		c[i] = c1 * a1[i] + c2 * a2[i];
}
void fast_add_aAbB_addto_C(mdp_complex c1, mdp_complex *a1, mdp_complex c2,
		mdp_complex *a2, mdp_complex *c, int &ni) {
	for (register int i = 0; i < ni * ni; i++)
		c[i] += c1 * a1[i] + c2 * a2[i];
}
void fast_add_aAbB_to_C(mdp_complex c1, mdp_complex *a1, mdp_complex c2,
		mdp_complex *a2, mdp_complex c3, mdp_complex *a3, mdp_complex *c,
		int &ni) {
	for (register int i = 0; i < ni * ni; i++)
...
void fast_add_aAbB_subto_C(mdp_complex c1, mdp_complex *a1, mdp_complex c2,
		mdp_complex *a2, mdp_complex *c, int &ni) {
	for (register int i = 0; i < ni * ni; i++)
		c[i] -= c1 * a1[i] + c2 * a2[i];
}
...
inline mdp_matrix staple(gauge_field &U, register site x, int mu) {
	...
	for (nu = 0; nu < U.ndim; nu++)
		if (nu != mu) {
			fast_mul_AB_to_C(&U(x, nu, 0, 0), &U(x + nu, mu, 0, 0), &b1(0, 0),
					nc);
			fast_mul_ABH_addto_C(&b1(0, 0), &U(x + mu, nu, 0, 0), &tmp(0, 0),
					nc);
			y = x - nu;
			fast_mul_AB_to_C(&U(y, mu, 0, 0), &U(y + mu, nu, 0, 0), &b1(0, 0),
					nc);
...
inline mdp_matrix staple_H(gauge_field &U, register site x, int mu, int s1,
	...
	site y(U.lattice());
	if (s1 == +1) {
		fast_mul_ABH_to_C(&U(x + mu, nu, 0, 0), &U(x + nu, mu, 0, 0), &b1(0, 0),
				nc);
...
inline mdp_matrix staple_H(gauge_field &U, register site x, int mu) {
	...
	for (nu = 0; nu < U.ndim; nu++)
		if (nu != mu) {
			fast_mul_ABH_to_C(&U(x + mu, nu, 0, 0), &U(x + nu, mu, 0, 0),
					&b1(0, 0), nc);
			fast_mul_ABH_addto_C(&b1(0, 0), &U(x, nu, 0, 0), &tmp(0, 0), nc);
			y = x - nu;
			fast_mul_AHB_to_C(&U(y, mu, 0, 0), &U(y, nu, 0, 0), &b1(0, 0), nc);
			fast_mul_AHB_addto_C(&U(y + mu, nu, 0, 0), &b1(0, 0), &tmp(0, 0),
					nc);
		}
	return tmp;
}
	...
	if (mu != 0)
		for (nu = 1; nu < U.ndim; nu++)
			if (nu != mu) {
				fast_mul_ABH_to_C(&U(x + mu, nu, 0, 0), &U(x + nu, mu, 0, 0),
						&b1(0, 0), nc);
				fast_mul_ABH_addto_C(&b1(0, 0), &U(x, nu, 0, 0), &tmp(0, 0),
						nc);
				y = x - nu;
				fast_mul_AHB_to_C(&U(y, mu, 0, 0), &U(y, nu, 0, 0), &b1(0, 0),
						nc);
				fast_mul_AHB_addto_C(&U(y + mu, nu, 0, 0), &b1(0, 0),
						&tmp(0, 0), nc);
			}
	return tmp;
}
