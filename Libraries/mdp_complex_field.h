...
bool mdp_write_double_as_float(FILE *fp, void* data, mdp_int psize,
...
bool mdp_read_double_as_float(FILE *fp, void* data, mdp_int psize,
		mdp_int header_size, mdp_int position, const mdp_lattice &lattice) {
	double *p = (double*) data;
	float *q = (float*) malloc(psize / 2);
	if (fseek(fp, position * psize / 2 + header_size, SEEK_SET)
...
bool mdp_write_float_as_double(FILE *fp, void* data, mdp_int psize,
		mdp_int header_size, mdp_int position, const mdp_lattice &lattice) {
	float *p = (float*) data;
...
bool mdp_read_float_as_double(FILE *fp, void* data, mdp_int psize,
		mdp_int header_size, mdp_int position, const mdp_lattice &lattice) {
	float *p = (float*) data;
	double *q = (double*) malloc(psize * 2);
	if (fseek(fp, position * psize * 2 + header_size, SEEK_SET)
...
public:
	...
	mdp_complex_field(const mdp_complex_field &other) :
			mdp_field<mdp_complex>(other) {
	}
	inline void operator=(const mdp_complex_field &psi) {
		if (&lattice() != &psi.lattice() || size != psi.size
	...
	inline friend mdp_real norm_square(mdp_complex_field &psi, int parity =
			EVENODD) {
		double n2 = 0;
	...
	inline friend mdp_complex scalar_product(mdp_complex_field &psi,
			mdp_complex_field &chi, int parity = EVENODD) {
		mdp_complex n2 = 0;
		mdp_int i_min = psi.physical_local_start(parity);
		mdp_int i_max = psi.physical_local_stop(parity);
		mdp_int i = i_min;
	...
	inline friend mdp_real real_scalar_product(mdp_complex_field &psi,
		...
		for (; i < i_max; i++)
			n2 += real(chi[i]) * real(psi[i]) + imag(chi[i]) * imag(psi[i]);
	...
	inline friend mdp_real imag_scalar_product(mdp_complex_field &psi,
		...
		for (; i < i_max; i++)
			n2 += real(psi[i]) * imag(chi[i]) + imag(psi[i]) * real(chi[i]);
		mpi.add(n2);
		return n2;
	}

	inline friend void mdp_add_scaled_field(mdp_complex_field &psi,
			mdp_real alpha, mdp_complex_field &chi, int parity = EVENODD) {
		mdp_int i_min = psi.physical_local_start(parity);
		mdp_int i_max = psi.physical_local_stop(parity);
		mdp_int i = i_min;
	...
	inline friend void mdp_add_scaled_field(mdp_complex_field &psi,
			mdp_complex alpha, mdp_complex_field &chi, int parity = EVENODD) {
		mdp_int i_min = psi.physical_local_start(parity);
		mdp_int i_max = psi.physical_local_stop(parity);
		mdp_int i = i_min;
	...
	inline friend mdp_real relative_residue(mdp_complex_field& p,
			mdp_complex_field& q, int parity = EVENODD) {
	...
	bool save_as_float(string filename, int processIO = 0,
	...
	bool load_as_float(string filename, int processIO = 0,
			mdp_int max_buffer_size = 1024, bool load_header = true,
			mdp_int skip_bytes = 0) {
		...
		header.bytes_per_site*=2;
#else
		load(filename, processIO, max_buffer_size, load_header, skip_bytes, 0,
				true);
#endif
		return true;
	}
