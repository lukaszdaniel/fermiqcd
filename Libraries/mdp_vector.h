...
public:
	...
	mdp_vector(int x0, int x1 = 0, int x2 = 0, int x3 = 0, int x4 = 0, int x5 =
			0, int x6 = 0, int x7 = 0, int x8 = 0, int x9 = 0) {
		x[0] = x0;
		x[1] = x1;
		x[2] = x2;
		x[3] = x3;
...
inline mdp_vector binary2versor(mdp_int a) {
	mdp_vector v((a) & 0x1, (a >> 1) & 0x1, (a >> 2) & 0x1, (a >> 3) & 0x1,
			(a >> 4) & 0x1, (a >> 5) & 0x1, (a >> 6) & 0x1, (a >> 7) & 0x1,
			(a >> 8) & 0x1, (a >> 9) & 0x1);
	return v;
}
...
#endif
	return x0 + 2 * x1 + 4 * x2 + 8 * x3 + 16 * x4 + 32 * x5 + 64 * x6
			+ 128 * x7 + 256 * x8 + 512 * x9;
}
...
inline mdp_int vector2binary(mdp_vector v) {
	...
	return v.x[0] + 2 * v.x[1] + 4 * v.x[2] + 8 * v.x[3] + 16 * v.x[4]
			+ 32 * v.x[5] + 64 * v.x[6] + 128 * v.x[7] + 256 * v.x[8]
			+ 512 * v.x[9];
}
