...
public:
	...
	/// Allows dynamic allocation of a field that is not allocated
	...
	*/
	...
	/// returns the address of the vector of objects T stored at site x
	...
	void operator=(const mdp_field &a) {
		if (&lattice() != &a.lattice() || size != a.size
	...
	mdp_int field_size() {
		return lattice().size() * field_components * Tsize;
	}
	...
	bool load(string filename, int processIO = 0,
			mdp_int max_buffer_size = 1024, bool load_header = true,
			mdp_int skip_bytes = 0,
			bool (*user_read)(FILE*, void*, mdp_int, mdp_int, mdp_int,
					const mdp_lattice&)=0, bool try_switch_endianess = true);

	bool save(string filename, int processIO = 0,
			mdp_int max_buffer_size = 1024, bool load_header = true,
			mdp_int skip_bytes = 0,
			bool (*user_write)(FILE*, void*, mdp_int, mdp_int, mdp_int,
					const mdp_lattice&)=0);

	bool save_vtk(string filename, int t = -1, int component = -1,
			int processIO = 0, bool ASCII = false);
