...
//////////////////////////////////////////////////////////////////
#ifndef mdp_matrix_test_
#define mdp_matrix_test_
...
bool mdp_matrix_test() {
	...
	assert(
			max(2 * a - mdp_complex(0, -1) * mdp_complex(0, 2) * a)
					< mdp_precision);
	printf("operator*                   ...test passed\n");

	assert(max(a + 3 * a - 4 * a) < mdp_precision);
	assert(
			max(a + mdp_complex(3, 2) * a - mdp_complex(4, 2) * a)
					< mdp_precision);
	printf("operator+ and operator-     ...test passed\n");

	assert(
			max(
					exp(mdp_complex(0, 1) * a) - cos(a)
							- mdp_complex(0, 1) * sin(a)) < mdp_precision);
