  ...
  public:
                      ...
                      coefficients &coeff, int parity = EVENODD)
    {
      ...
      if (coeff.has_key("kappa"))
        kappat = kappas = coeff["kappa"];
      if (coeff.has_key("zeta"))
        kappat = kappas / coeff["zeta"];

      if (coeff.has_key("kappa_s"))
        kappas = coeff["kappa_s"];
      if (coeff.has_key("kappa_t"))
        kappat = coeff["kappa_t"];
      ...
      if (coeff.has_key("c_E"))
        ...
        cE = 1; // this definition different from Clover

      if (coeff.has_key("c_B"))
        cB = coeff["c_B"];
      else
        cB = 1; // this definition different from Clover

      if (coeff.has_key("alpha_1"))
        c1 = coeff["alpha_1"];
      else
        c1 = 0;
      if (coeff.has_key("alpha_2"))
        c2 = coeff["alpha_2"];
      else
        c2 = 0;
      if (coeff.has_key("alpha_3"))
        c3 = coeff["alpha_3"];
      else
        c3 = 0;
      if (coeff.has_key("alpha_4"))
        c4 = coeff["alpha_4"];
      else
        c4 = 0;
      if (coeff.has_key("alpha_5"))
        c5 = coeff["alpha_5"];
      else
        c5 = 0;
      ...
      if (coeff.has_key("alpha_10"))
        ...
        c10 = 0;
      if (coeff.has_key("mac_1"))
        mac1 = coeff["mac_1"];
      else
        mac1 = 0;
      if (coeff.has_key("mac_2"))
        mac2 = coeff["mac_2"];
      else
        mac2 = 0;
      if (coeff.has_key("mac_3"))
        mac3 = coeff["mac_3"];
      else
        mac3 = 0;
      if (coeff.has_key("mac_4"))
        mac4 = coeff["mac_4"];
      else
        mac4 = 0;
      if (coeff.has_key("mac_5"))
        mac5 = coeff["mac_5"];
      else
        mac5 = 0;
      ...
      for (i = 1; i < 4; i++)
        for (j = 1; j < 4; j++)
        {
          ...
          M = c1 * kappat * (Gamma[i] + Gamma[j]) + c2 * kappat * Gamma[i] * delta(i, j) + 2.0 * c3 * kappat + 2.0 * c4 * kappat * delta(i, j);
          for (a = 0; a < 4; a++)
            for (b = 0; b < 4; b++)
            {
      ...
      for (a = 0; a < 4; a++)
      {
        ...
        forallsites(x)
        {
          ...
          for (b = 0; b < 4; b++)
          {
            ...
            k = 16 * a + 4 * b + 0;
            if (c_u[k] != 0)
              _sse_mulAbC_add_31(&Up(x, 0, 0), c_u[k], &out(0, 0));
            if (c_d[k] != 0)
              _sse_mulAbC_add_31(&Dw(x, 0, 0), c_d[k], &out(0, 0));
            k = 16 * a + 4 * b + 1;
            if (c_u[k] != 0)
              _sse_mulAbC_add_31(&Up(x, 1, 0), c_u[k], &out(0, 0));
            if (c_d[k] != 0)
              _sse_mulAbC_add_31(&Dw(x, 1, 0), c_d[k], &out(0, 0));
            if (c_e[k] != 0)
              _sse_mulAbC_add_31(&Ei(x, 0, 0), c_e[k], &out(0, 0));
            if (c_b[k] != 0)
              _sse_mulAbC_add_31(&Bi(x, 0, 0), c_b[k], &out(0, 0));
            k = 16 * a + 4 * b + 2;
            if (c_u[k] != 0)
              _sse_mulAbC_add_31(&Up(x, 2, 0), c_u[k], &out(0, 0));
            if (c_d[k] != 0)
              _sse_mulAbC_add_31(&Dw(x, 2, 0), c_d[k], &out(0, 0));
            if (c_e[k] != 0)
              _sse_mulAbC_add_31(&Ei(x, 1, 0), c_e[k], &out(0, 0));
            if (c_b[k] != 0)
              _sse_mulAbC_add_31(&Bi(x, 1, 0), c_b[k], &out(0, 0));
            k = 16 * a + 4 * b + 3;
            if (c_u[k] != 0)
              _sse_mulAbC_add_31(&Up(x, 3, 0), c_u[k], &out(0, 0));
            if (c_d[k] != 0)
              _sse_mulAbC_add_31(&Dw(x, 3, 0), c_d[k], &out(0, 0));
            if (c_e[k] != 0)
              _sse_mulAbC_add_31(&Ei(x, 2, 0), c_e[k], &out(0, 0));
            if (c_b[k] != 0)
              _sse_mulAbC_add_31(&Bi(x, 2, 0), c_b[k], &out(0, 0));
            k = 64 * a + 16 * b + 4 * 0 + 0;
            if (c_uu[k] != 0)
              _sse_mulAbC_add_31(&u0u0(0, 0), c_uu[k], &out(0, 0));
            if (c_dd[k] != 0)
              _sse_mulAbC_add_31(&d0d0(0, 0), c_dd[k], &out(0, 0));
            k = 64 * a + 16 * b + 4 * 1 + 1;
            if (c_uu[k] != 0)
              _sse_mulAbC_add_31(&u1u1(0, 0), c_uu[k], &out(0, 0));
            if (c_dd[k] != 0)
              _sse_mulAbC_add_31(&d1d1(0, 0), c_dd[k], &out(0, 0));
            if (c_ub[k] != 0)
              _sse_mulAbC_add_31(&u1b1(0, 0), c_ub[k], &out(0, 0));
            if (c_db[k] != 0)
              _sse_mulAbC_add_31(&d1b1(0, 0), c_db[k], &out(0, 0));
            if (c_bu[k] != 0)
              _sse_mulAbC_add_31(&b1u1(0, 0), c_bu[k], &out(0, 0));
            if (c_bd[k] != 0)
              _sse_mulAbC_add_31(&b1d1(0, 0), c_bd[k], &out(0, 0));
            k = 64 * a + 16 * b + 4 * 1 + 2;
            if (c_uu[k] != 0)
              _sse_mulAbC_add_31(&u1u2(0, 0), c_uu[k], &out(0, 0));
            if (c_dd[k] != 0)
              _sse_mulAbC_add_31(&d1d2(0, 0), c_dd[k], &out(0, 0));
            if (c_ud[k] != 0)
              _sse_mulAbC_add_31(&u1d2(0, 0), c_ud[k], &out(0, 0));
            if (c_du[k] != 0)
              _sse_mulAbC_add_31(&d1u2(0, 0), c_du[k], &out(0, 0));
            if (c_ub[k] != 0)
              _sse_mulAbC_add_31(&u1b2(0, 0), c_ub[k], &out(0, 0));
            if (c_db[k] != 0)
              _sse_mulAbC_add_31(&d1b2(0, 0), c_db[k], &out(0, 0));
            if (c_bu[k] != 0)
              _sse_mulAbC_add_31(&b1u2(0, 0), c_bu[k], &out(0, 0));
            if (c_bd[k] != 0)
              _sse_mulAbC_add_31(&b1d2(0, 0), c_bd[k], &out(0, 0));
            k = 64 * a + 16 * b + 4 * 1 + 3;
            if (c_uu[k] != 0)
              _sse_mulAbC_add_31(&u1u3(0, 0), c_uu[k], &out(0, 0));
            if (c_dd[k] != 0)
              _sse_mulAbC_add_31(&d1d3(0, 0), c_dd[k], &out(0, 0));
            if (c_ud[k] != 0)
              _sse_mulAbC_add_31(&u1d3(0, 0), c_ud[k], &out(0, 0));
            if (c_du[k] != 0)
              _sse_mulAbC_add_31(&d1u3(0, 0), c_du[k], &out(0, 0));
            if (c_ub[k] != 0)
              _sse_mulAbC_add_31(&u1b3(0, 0), c_ub[k], &out(0, 0));
            if (c_db[k] != 0)
              _sse_mulAbC_add_31(&d1b3(0, 0), c_db[k], &out(0, 0));
            if (c_bu[k] != 0)
              _sse_mulAbC_add_31(&b1u3(0, 0), c_bu[k], &out(0, 0));
            if (c_bd[k] != 0)
              _sse_mulAbC_add_31(&b1d3(0, 0), c_bd[k], &out(0, 0));
            k = 64 * a + 16 * b + 4 * 2 + 1;
            if (c_uu[k] != 0)
              _sse_mulAbC_add_31(&u2u1(0, 0), c_uu[k], &out(0, 0));
            if (c_dd[k] != 0)
              _sse_mulAbC_add_31(&d2d1(0, 0), c_dd[k], &out(0, 0));
            if (c_ud[k] != 0)
              _sse_mulAbC_add_31(&u2d1(0, 0), c_ud[k], &out(0, 0));
            if (c_du[k] != 0)
              _sse_mulAbC_add_31(&d2u1(0, 0), c_du[k], &out(0, 0));
            if (c_ub[k] != 0)
              _sse_mulAbC_add_31(&u2b1(0, 0), c_ub[k], &out(0, 0));
            if (c_db[k] != 0)
              _sse_mulAbC_add_31(&d2b1(0, 0), c_db[k], &out(0, 0));
            if (c_bu[k] != 0)
              _sse_mulAbC_add_31(&b2u1(0, 0), c_bu[k], &out(0, 0));
            if (c_bd[k] != 0)
              _sse_mulAbC_add_31(&b2d1(0, 0), c_bd[k], &out(0, 0));
            k = 64 * a + 16 * b + 4 * 2 + 2;
            if (c_uu[k] != 0)
              _sse_mulAbC_add_31(&u2u2(0, 0), c_uu[k], &out(0, 0));
            if (c_dd[k] != 0)
              _sse_mulAbC_add_31(&d2d2(0, 0), c_dd[k], &out(0, 0));
            if (c_ub[k] != 0)
              _sse_mulAbC_add_31(&u2b2(0, 0), c_ub[k], &out(0, 0));
            if (c_db[k] != 0)
              _sse_mulAbC_add_31(&d2b2(0, 0), c_db[k], &out(0, 0));
            if (c_bu[k] != 0)
              _sse_mulAbC_add_31(&b2u2(0, 0), c_bu[k], &out(0, 0));
            if (c_bd[k] != 0)
              _sse_mulAbC_add_31(&b2d2(0, 0), c_bd[k], &out(0, 0));
            k = 64 * a + 16 * b + 4 * 2 + 3;
            if (c_uu[k] != 0)
              _sse_mulAbC_add_31(&u2u3(0, 0), c_uu[k], &out(0, 0));
            if (c_dd[k] != 0)
              _sse_mulAbC_add_31(&d2d3(0, 0), c_dd[k], &out(0, 0));
            if (c_ud[k] != 0)
              _sse_mulAbC_add_31(&u2d3(0, 0), c_ud[k], &out(0, 0));
            if (c_du[k] != 0)
              _sse_mulAbC_add_31(&d2u3(0, 0), c_du[k], &out(0, 0));
            if (c_ub[k] != 0)
              _sse_mulAbC_add_31(&u2b3(0, 0), c_ub[k], &out(0, 0));
            if (c_db[k] != 0)
              _sse_mulAbC_add_31(&d2b3(0, 0), c_db[k], &out(0, 0));
            if (c_bu[k] != 0)
              _sse_mulAbC_add_31(&b2u3(0, 0), c_bu[k], &out(0, 0));
            if (c_bd[k] != 0)
              _sse_mulAbC_add_31(&b2d3(0, 0), c_bd[k], &out(0, 0));
            k = 64 * a + 16 * b + 4 * 3 + 1;
            if (c_uu[k] != 0)
              _sse_mulAbC_add_31(&u3u1(0, 0), c_uu[k], &out(0, 0));
            if (c_dd[k] != 0)
              _sse_mulAbC_add_31(&d3d1(0, 0), c_dd[k], &out(0, 0));
            if (c_ud[k] != 0)
              _sse_mulAbC_add_31(&u3d1(0, 0), c_ud[k], &out(0, 0));
            if (c_du[k] != 0)
              _sse_mulAbC_add_31(&d3u1(0, 0), c_du[k], &out(0, 0));
            if (c_ub[k] != 0)
              _sse_mulAbC_add_31(&u3b1(0, 0), c_ub[k], &out(0, 0));
            if (c_db[k] != 0)
              _sse_mulAbC_add_31(&d3b1(0, 0), c_db[k], &out(0, 0));
            if (c_bu[k] != 0)
              _sse_mulAbC_add_31(&b3u1(0, 0), c_bu[k], &out(0, 0));
            if (c_bd[k] != 0)
              _sse_mulAbC_add_31(&b3d1(0, 0), c_bd[k], &out(0, 0));
            k = 64 * a + 16 * b + 4 * 3 + 2;
            if (c_uu[k] != 0)
              _sse_mulAbC_add_31(&u3u2(0, 0), c_uu[k], &out(0, 0));
            if (c_dd[k] != 0)
              _sse_mulAbC_add_31(&d3d2(0, 0), c_dd[k], &out(0, 0));
            if (c_ud[k] != 0)
              _sse_mulAbC_add_31(&u3d2(0, 0), c_ud[k], &out(0, 0));
            if (c_du[k] != 0)
              _sse_mulAbC_add_31(&d3u2(0, 0), c_du[k], &out(0, 0));
            if (c_ub[k] != 0)
              _sse_mulAbC_add_31(&u3b2(0, 0), c_ub[k], &out(0, 0));
            if (c_db[k] != 0)
              _sse_mulAbC_add_31(&d3b2(0, 0), c_db[k], &out(0, 0));
            if (c_bu[k] != 0)
              _sse_mulAbC_add_31(&b3u2(0, 0), c_bu[k], &out(0, 0));
            if (c_bd[k] != 0)
              _sse_mulAbC_add_31(&b3d2(0, 0), c_bd[k], &out(0, 0));
            k = 64 * a + 16 * b + 4 * 3 + 3;
            if (c_uu[k] != 0)
              _sse_mulAbC_add_31(&u3u3(0, 0), c_uu[k], &out(0, 0));
            if (c_dd[k] != 0)
              _sse_mulAbC_add_31(&d3d3(0, 0), c_dd[k], &out(0, 0));
            if (c_ub[k] != 0)
              _sse_mulAbC_add_31(&u3b3(0, 0), c_ub[k], &out(0, 0));
            if (c_db[k] != 0)
              _sse_mulAbC_add_31(&d3b3(0, 0), c_db[k], &out(0, 0));
            if (c_bu[k] != 0)
              _sse_mulAbC_add_31(&b3u3(0, 0), c_bu[k], &out(0, 0));
            if (c_bd[k] != 0)
              _sse_mulAbC_add_31(&b3d3(0, 0), c_bd[k], &out(0, 0));
