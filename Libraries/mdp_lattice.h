...
public:
	...
	int next_next; /* 1 or 2 is the thickness of the boundary */
...
public:
	...
	mdp_lattice(int ndim_, int nx_[],
			...
			=torus_topology, mdp_int random_seed_ = 0, int next_next_ = 1,
			bool local_random_ = true) {
		nvol = 0;
		random_obj = 0;
		allocate_lattice(ndim_, ndim_, nx_, where_, neighbour_, random_seed_,
				next_next_, local_random_);
	}
	/// for weird stuff
	mdp_lattice(int ndim_, int ndir_, int nx_[],
			int (*where_)(int*, int, int*)=default_partitioning0,
			void (*neighbour_)(int, int*, int*, int*, int, int*)
			=torus_topology, mdp_int random_seed_ = 0, int next_next_ = 1,
			bool local_random_ = true) {
		nvol = 0;
		random_obj = 0;
		allocate_lattice(ndim_, ndir_, nx_, where_, neighbour_, random_seed_,
				next_next_, local_random_);
	}
	...
	/// @param local_random_ true is local random generator is required 
	void allocate_lattice(int ndim_, int nx_[],
			int (*where_)(int*, int, int*)=default_partitioning0,
			void (*neighbour_)(int, int*, int*, int*, int, int*)
			=torus_topology, mdp_int random_seed_ = 0, int next_next_ = 1,
			bool local_random_ = true) {
	...
	/// for weird stuff
	void allocate_lattice(int ndim_, int ndir_, int nx_[],
			int (*where_)(int*, int, int*)=default_partitioning0,
			void (*neighbour_)(int, int*, int*, int*, int, int*)
			=torus_topology, mdp_int random_seed_ = 0, int next_next_ = 1,
			bool local_random_ = true) {
						...
						if ((nu != mu) || (next_next > 1)) {
							...
							if (next_next == 3)
								for (rho = 0; rho < ndir; rho++) {
									(*neighbour)(rho, x_dw_dw_dw, x_dw_dw,
											x_dw_dw_up, ndim, nx);
									(*neighbour)(rho, x_up_up_dw, x_up_up,
											x_up_up_up, ndim, nx);
									if (((*where)(x_up_up_up, ndim, nx) >= Nproc)
											|| ((*where)(x_up_up_dw, ndim, nx)
													>= Nproc)
											|| ((*where)(x_dw_dw_up, ndim, nx)
													>= Nproc)
											|| ((*where)(x_dw_dw_dw, ndim, nx)
													>= Nproc))
		...
		if (mdp_random_seed_filename && random_seed_ == 0) {
			if (ME == 0) {
				...
				mpi << "Reading from file " << mdp_random_seed_filename
						<< " lattice().random_seed=" << random_seed_ << '\n';
				mpi << "Writing to   file " << mdp_random_seed_filename
						<< " lattice().random_seed=" << random_seed_ + 1
						<< '\n';
			}
	...
	void initialize_random(mdp_int random_seed_ = 0) {
		...
		if (local_random_generator) {
			...
			for (mdp_int idx = 0; idx < nvol_in; idx++)
				random_obj[idx].initialize(
						gl[idx + start[ME][0]] + random_seed);
		}
	}
