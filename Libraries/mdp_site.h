...
int on_which_process(mdp_lattice &a, int x0 = 0, int x1 = 0, int x2 = 0,
		int x3 = 0, int x4 = 0, int x5 = 0, int x6 = 0, int x7 = 0, int x8 = 0,
		int x9 = 0);
...
public:
	...
	/// the current process
	inline int is_in() {
		if ((idx >= lattice().start[ME][0]) && (idx < lattice().stop[ME][1]))
			return 1;
		else
			return 0;
	}
	...
	inline mdp_site operator=(mdp_vector v) {
		set(v.x[0], v.x[1], v.x[2], v.x[3], v.x[4], v.x[5], v.x[6], v.x[7],
				v.x[8], v.x[9]);
		return *this;
	}
	...
	void set(int x0, int x1 = 0, int x2 = 0, int x3 = 0, int x4 = 0, int x5 = 0,
			int x6 = 0, int x7 = 0, int x8 = 0, int x9 = 0) {
		int ndim = lattice().ndim;
		idx = x0;
		if (ndim > 1)
			idx = idx * lattice().nx[1] + x1;
		if (ndim > 2)
			idx = idx * lattice().nx[2] + x2;
		if (ndim > 3)
			idx = idx * lattice().nx[3] + x3;
		if (ndim > 4)
			idx = idx * lattice().nx[4] + x4;
		if (ndim > 5)
			idx = idx * lattice().nx[5] + x5;
		if (ndim > 6)
			idx = idx * lattice().nx[6] + x6;
		if (ndim > 7)
			idx = idx * lattice().nx[7] + x7;
		if (ndim > 8)
			idx = idx * lattice().nx[8] + x8;
		if (ndim > 9)
			idx = idx * lattice().nx[9] + x9;
		idx = lattice().local(idx);
		if (idx == NOWHERE) {
			bool print = mdp.print;
			mdp.print = true;
