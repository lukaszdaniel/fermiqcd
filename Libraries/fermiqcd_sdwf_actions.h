...
public:
	static void mul_Q(sdwf_field &chi_out, sdwf_field &chi_in, gauge_field &U,
		...
		if (coeff.has_key("m_0"))
			mass = coeff["m_0"];
		else
			error("coefficient m_0 undeclared");
		if (coeff.has_key("m_f"))
			m_f = coeff["m_f"];
		else
			error("coefficient m_f undeclared");
		if (coeff.has_key("m_5"))
			m_5 = coeff["m_5"];
		else
			error("coefficient m_5 undeclared");
		...
		for (x5 = 0; x5 < L5; x5++) {
			...
			for(i=0; i < nc;
				...
				forallsitesofparity(x, parity)
				{
					...
					for (i = 0; i < nc; i++)
						for (j = 0; j < nc; j++)
							chi_out(x, 0, i) -= phase * FU_dw[nc * i + j]
									* Fchi_dw[j];
