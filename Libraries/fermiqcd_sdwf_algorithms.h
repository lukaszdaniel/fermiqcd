...
void project(staggered_field &psi, sdwf_field &chi, gauge_field &U) {
	...
	forallsites(x)
	{
		...
		psi(x) = 0.5 * chi(x, 0)
				- (MDP_SDWF_SGN * 0.5 * phase) * (U.swirls(x) * chi(y, 0))
				+ 0.5 * chi(x, L5 - 1)
				+ (MDP_SDWF_SGN * 0.5 * phase) * (U.swirls(x) * chi(y, L5 - 1));
	}
}
...
void project(staggered_field &psi, sdwf_field &chi, gauge_field &U, int sign, int L) {
	...
	forallsites(x)
	{
		...
		y = chi.chiral_shift(x);
		psi(x) = 0.5 * chi(x, L)
				- (0.5 * phase * sign) * (U.swirls(x) * chi(y, L));
	}
}
...
void project(sdwf_field &chi, staggered_field &psi, gauge_field &U) {
	...
	forallsites(x)
	{
		...
		y = chi.chiral_shift(x);
		chi(x, 0) = 0.5 * psi(x)
				- (MDP_SDWF_SGN * 0.5 * phase) * (U.swirls(x) * psi(y));
		chi(x, chi.L5 - 1) = 0.5 * psi(x)
				+ (MDP_SDWF_SGN * 0.5 * phase) * (U.swirls(x) * psi(y));
	}
}
...
void (*default_sdwf_action)(sdwf_field &, sdwf_field &, gauge_field &,
		coefficients &, int) = SDWFActionSlow::mul_Q;

void mul_Q(sdwf_field &psi_out, sdwf_field &psi_in, gauge_field &U,
...
inversion_stats (*default_sdwf_inverter)(sdwf_field &, sdwf_field &,
		gauge_field &, coefficients &, mdp_real, mdp_real, int)
		=BiConjugateGradientStabilizedInverter<sdwf_field,gauge_field>;

inversion_stats mul_invQ(sdwf_field &psi_out, sdwf_field &psi_in,
		gauge_field &U, coefficients &coeff, mdp_real absolute_precision =
				sdwf_inversion_precision, mdp_real relative_precision = 0,
		int max_steps = 2000) {
	return (*default_sdwf_inverter)(psi_out, psi_in, U, coeff,
