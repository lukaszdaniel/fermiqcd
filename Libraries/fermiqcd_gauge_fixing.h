...
public:
	...
	static void hit(gauge_field &U, int mu, int parity, int i, int j,
		...
		forallsitesofparity(x, parity)
		{
			...
			for (nu = 0; nu < U.ndim; nu++)
				if (nu != mu) {
					U_up = U(x, nu);
					U_dw = U(x - nu, nu);
					a0 += +real(U_dw(i, i)) + real(U_up(i, i))
							+ real(U_dw(j, j)) + real(U_up(j, j));
					a1 += +imag(U_dw(j, i)) - imag(U_up(j, i))
							+ imag(U_dw(i, j)) - imag(U_up(i, j));
					a2 += -real(U_dw(j, i)) + real(U_up(j, i))
							+ real(U_dw(i, j)) - real(U_up(i, j));
					a3 += +imag(U_dw(i, i)) - imag(U_up(i, i))
	...
	static void z3_fix(gauge_field &U, int mu) {
		...
		site x(U.lattice());
		mdp_matrix A;
		mdp_complex phase[3] = { mdp_complex(1, 0), exp(2.0 * Pi * I / 3.0),
				exp(4.0 * Pi * I / 3.0) };
		mdp_real alpha[3];
	...
	///           torus topology
	static gaugefixing_stats fix(gauge_field &U, int mu = 0, int max_steps = 1,
			mdp_real target_precision = 1e-5, mdp_real overrelaxation_boost = 1,
			bool z3 = false) {
		...
		for (step = 0; step < max_steps; step++) {
			...
			precision = sqrt(
					precision / (U.nc * U.nc * U.lattice().global_volume()));
		...
		mdp << "steps=" << step << ", action=" << action << ", precison+"
				<< precision << '\n';
