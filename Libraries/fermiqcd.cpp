...
void cool(gauge_field& U, mdp_args& arguments) {
	if (arguments.get("-cool", "alg", "ape") == "ape")
		ApeSmearing::smear(U, arguments.get("-cool", "alpha", 0.7),
...
void cool_vtk(gauge_field& U, mdp_args& arguments, string filename) {
	if (arguments.get("-cool", "alg", "ape") == "ape")
		for (int k = 0; k < arguments.get("-cool_vtk", "n", 20); k++) {
			ApeSmearing::smear(U, arguments.get("-cool_vtk", "alpha", 0.7),
					arguments.get("-cool_vtk", "steps", 1),
					arguments.get("-cool_vtk", "cooling", 10));
			topological_charge_vtk(U,
					filename + ".cool" + tostring(k, 2) + ".vtk", 0);
		}
	else
		mdp.error_message("cooling algorithm not supported");
}
...
void polyakov_vtk(gauge_field& U, string filename) {
	...
	mdp_lattice space(3, L, default_partitioning<0>, torus_topology, 0, 1,
			false);
...
void make_quark(gauge_field &U, coefficients &gauge, coefficients &quark,
	...
	string quark_action = arguments.get("-quark", "action",
			"clover_fast|clover_slow|clover_sse2");
	string inverter = arguments.get("-quark", "alg",
			"bicgstab|minres|bicgstabvtk|minresvtk");
	...
	bool use_propagator = arguments.have("-current_static")
			|| arguments.have("-meson") || arguments.have("-wave_static")
		...
		for (int i = 0; i < nc; i++) {
			...
			if (source_type == "point") {
				...
				forallsites(x)
				{
					...
					{
						psi(x, b, j) =
								(x(TIME) == t0 && b == a && j == i) ? 1 : 0;
					}
				}
			}
			// optional ... smer source here
			psi.update();
			prefix = newfilename + "." + source_type + ".k"
					+ tostring(quark["kappa"]);
			if (t0 * t0 + x0 * x0 + y0 * y0 + z0 * z0 > 0)
				prefix = prefix + ".at" + tostring(t0) + "." + tostring(x0)
						+ "." + tostring(y0) + "." + tostring(z0);
			inversion_vtk_prefix = prefix + ".s" + tostring(a, 1) + ".c"
					+ tostring(i, 1);
			...
			if (arguments.get("-quark", "vtk", "false|true") == "true") {
				forspincolor(b,j,nc)
				{
					forallsites(x)
						Q(x) = abs(phi(x, b, j));
					Q.save_vtk(
							inversion_vtk_prefix + ".quark" + tostring(b, 1)
									+ tostring(j, 1) + ".vtk", -1);
				}
			}
	...
	if (arguments.have("-meson")) {
		...
		G1 = Gamma5
				* parse_gamma(
						arguments.get("-meson", "source_gamma",
								"5|0|1|2|3|01|02|03|12|13|05|15|25|35|I"));
		G2 = parse_gamma(
				arguments.get("-meson", "sink_gamma",
						"5|0|1|2|3|01|02|03|12|13|05|15|25|35|I")) * Gamma5;
	...
	if (arguments.have("-current_static")) {
		...
		Q = 0;
		G1 = parse_gamma(
				arguments.get("-current_static", "source_gamma",
						"5|0|1|2|3|01|02|03|12|13|05|15|25|35|I")) * Gamma5;
		G2 = parse_gamma(
				arguments.get("-current_static", "sink_gamma",
						"5|0|1|2|3|01|02|03|12|13|05|15|25|35|I"));
		G3 = Gamma5
				* parse_gamma(
						arguments.get("-current_static", "current_gamma",
								"I|0|1|2|3|5|01|02|03|12|13|05|15|25|35"));
	...
	if (arguments.have("-4quark")) {
		mdp_matrix G = parse_gamma(
				arguments.get("-4quark", "source",
						"5|I|0|1|2|3|05|15|25|35|01|02|03|12|13|23")) * Gamma5;
		forspincolor(a,i,U.nc)
		{
			forspincolor(b,j,U.nc)
			{
				...
				for (int c = 0; c < 4; c++)
					for (int d = 0; d < 4; d++)
						if (G(c, d) != 0)
							forallsites(x)
								for (int k = 0; k < U.nc; k++)
									open_prop[a][b][i][j][(x(TIME) - t0 + NT)
											% NT] += S(x, a, c, i, k)
											* conj(S(x, b, d, j, k)) * G(c, d);
				mpi.add((mdp_complex*) &open_prop[0], NT * 4 * 4 * 3 * 3);
			}
		}
		mdp_matrix G1, G2;
		string op4q =
				arguments.get("-4quark", "operator",
						"5Ix5I|0Ix0I|1Ix1I|2Ix2I|3Ix3I|05Ix05I|15Ix15I|25Ix25I|35Ix35I|01Ix01I|02Ix02I|03Ix03I|12Ix12I|13Ix13I|23Ix23I|5Tx5T|0Tx0T|1Tx1T|2Tx2T|3Tx3T|05Tx05T|15Tx15T|25Tx25T|35Tx35T|01Tx01T|02Tx02T|03Tx03T|12Tx12T|13Tx13T|23Tx23T");
		bool rotate = false;
									...
									for (int i = 0; i < U.nc; i++)
										for (int j = 0; j < U.nc; j++)
											if (!rotate) {
												c3a +=
														real(
																open_prop[a][b][i][i][t1s]
																		* g1
																		* open_prop[c][d][j][j][t2s]
																		* g2);
												c3b +=
														real(
																open_prop[c][b][j][i][t1s]
																		* g1
																		* open_prop[a][d][i][j][t2s]
																		* g2);
											} else
												for (int z = 1; z < 9; z++)
													for (int k1 = 0; k1 < U.nc;
															k1++)
														for (int k2 = 0;
																k2 < U.nc;
																k2++) {
															c3a +=
																	real(
																			open_prop[a][b][i][k1][t1s]
																					* g1
																					* Lambda[z](
																							k1,
																							i)
																					* open_prop[c][d][j][k2][t2s]
																					* g2
																					* Lambda[z](
																							k2,
																							j))
															...
															c3b +=
																	real(
																			open_prop[c][b][j][k1][t1s]
																					* g1
																					* Lambda[z](
																							k1,
																							i)
																					* open_prop[a][d][i][k2][t2s]
																					* g2
																					* Lambda[z](
																							k2,
																							j))
	...
	if (arguments.have("-wave_static")) {
		...
		string source_gamma = arguments.get("-wave_static", "source_gamma",
				"5|0|1|2|3|01|02|03|12|13|05|15|25|35|I");
...
int main(int argc, char** argv) {
	...
	size[2] = ny;
	size[3] = nz;
	string gauge_start = arguments.get("-gauge", "start",
			"load|cold|hot|instantons");
	if (gauge_start == "cold")
	...
	mdp_lattice lattice(ndim, size, default_partitioning<1>, torus_topology, 0,
			1, (gauge_start != "load" || nconfigs > 0));
	...
	for (int f = 0; f < filenames.size(); f++) {
		...
		if (gauge_start == "cold") {
			...
			instantons.push_back(
					SingleInstanton4D(t0, x0, y0, z0, abs(r0),
							(r0 > 0) ? +1 : -1));
			if (r1 != 0)
				instantons.push_back(
						SingleInstanton4D(t1, x1, y1, z1, abs(r1),
								(r1 > 0) ? +1 : -1));
		...
		for (int n = -1; n < nconfigs; n++) {
			...
			if (arguments.have("-topcharge_vtk")) {
				float tc = topological_charge_vtk(U,
						newfilename + ".topcharge.vtk", -1);
