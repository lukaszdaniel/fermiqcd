...
private:
	...
	enum enumSendRecv {
		LOG_SR_SEND, LOG_SR_RECV
	};
	enum enumSendRecvStep {
		LOG_SR_START, LOG_SR_FAIL, LOG_SR_SUCCESS
	};
	...
	void psim_begin(int processCount, string logFileName, int verbatim) {
		...
		open_log();
		if ((processCount < PROCESS_COUNT_MIN)
				|| (processCount < PROCESS_COUNT_MIN)) {
		...
		char buffer[256];
		sprintf(buffer, "process %i of %i created with pid=%i", _processID,
				_processCount, getpid());
		log(buffer, 1);
	...
	void create_sockets() {
			...
			for (int dest = 0; dest < _processCount; dest++) {
				...
				while (mkfifo(filename, 0666) < 0) {
					if (errno == EEXIST)
						...
						throw string("PSIM ERROR: unable to mkfifo ")
								+ filename;
				}
				_socketFD[_processCount * source + dest][COMM_RECV] = open(
						filename, O_RDONLY | O_NONBLOCK);
				_socketFD[_processCount * source + dest][COMM_SEND] = open(
						filename, O_WRONLY);
				int flags = fcntl(
						_socketFD[_processCount * source + dest][COMM_RECV],
						F_GETFL, 0);
				fcntl(_socketFD[_processCount * source + dest][COMM_RECV],
		...
		for (int source = 0; source < _processCount; source++)
			for (int dest = 0; dest < _processCount; dest++) {
				sprintf(buffer, "_socketFD[%i*%i+%i]={%i,%i}", source,
						_processCount, dest,
	...
	// ***                                                             *** 
	// ***  Verifies that the destination process ID is valid. This    ***
	...
	void check_process_id(int processID) {
		...
		if ((processID == _processID) || (processID < PROCESS_PARENT)
	...
	void logSendRecv(int sourcedestProcessID, string tag, enumSendRecv method,
			enumSendRecvStep step) {

		char buffer[256];
		const char cmdSendRecv[2][8] = { "send", "recv" };
		const char stepSendRecv[3][12] =
				{ "starting...", "failed!", "success." };

		sprintf(buffer, "%i %s(%i,%s) %s", _processID, cmdSendRecv[method],
	...
	void send_buffer(int destProcessID, const void* pdataToSend,
			mdp_int dataSize) {
		...
		if (write(_socketFD[destIndex][COMM_SEND], &counter, sizeof(counter))
				!= sizeof(counter)) {
			log("PSIM ERROR: failure to write to socket");
			throw string("PSIM ERROR: failure to write to socket");
		}
	...
	void send_binary(int destProcessID, const string& tag,
	...
	void recv_buffer(int sourceProcessID, void* pdataToReceive,
			mdp_int dataSize) {
		char filename[512];
		int counter;
		int sourceIndex = get_source_index(sourceProcessID);
		if (read(_socketFD[sourceIndex][COMM_RECV], &counter, sizeof(counter))
				!= sizeof(counter)) {
			log("PSIM ERROR: timeout error in reading from socket");
			throw string("PSIM ERROR: timeout error in reading from socket");
		}
		sprintf(filename, ".fifo.%i.%i.%i", sourceProcessID, _processID,
				counter);
	...
	void recv_binary(int sourceProcessID, const string& tag,
			vector<char>& data) {
...
public:
	...
	mdp_psim(int processCount, string logFileName = ".psim.log", int verbatim =
			0) {
	...
	// ***         Public Method: send                                 *** 
	// ***                                                             *** 
	// ***  This asynchronous method sends the data referenced bu      ***
	...
	template<class T>
	void send(int destProcessID, string dataTag, T *pdataToSend,
			mdp_int dataSize) {
		logSendRecv(destProcessID, dataTag, LOG_SR_SEND, LOG_SR_START);
	...
	template<class T>
	void recv(int sourceProcessID, string dataTag, T *pdataToReceive,
			mdp_int dataSize) {
		logSendRecv(sourceProcessID, dataTag, LOG_SR_RECV, LOG_SR_START);
		vector<char> data;
		recv_binary(sourceProcessID, dataTag, data);
	...
	vector<T> combine(T &data) {
		...
		cout << id() << " list=" << dataList[0] << dataList[1] << dataList[2]
				<< endl;
		return dataList;
	}
