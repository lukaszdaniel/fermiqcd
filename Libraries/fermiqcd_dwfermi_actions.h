...
public:
	static void mul_Q(dwfermi_field &psi_out, dwfermi_field &psi_in,
			gauge_field &U, coefficients &coeff) {

		if (psi_in.nspin != 4)
			error("fermiqcd_dwfermi_algorithms/dwfermi_mul_Q_ONE: nspin!=4");
		if (psi_in.nc != U.nc)
			error(
					"fermiqcd_dwfermi_algorithms/dwfermi_mul_Q_ONE: gauge and spinor have different nc");
			...
			for (l = 0; l < L5; l++) {
				for (mu = 0; mu < ndim; mu++) {
					for (a = 0; a < nspin; a++) {
						...
						psi_dw(a) = hermitian(U(x - mu, mu))
								* psi_in(x - mu, l, a);
					}
					psi_out(x, l) += kappa5
							* ((1 - sign * Gamma[mu]) * psi_up
				...
				for (mu = 0; mu < ndim; mu++) {
					for (a = 0; a < nspin; a++)
						for (int i = 0; i < nc; i++) {
							...
							for (int j = 0; j < nc; j++) {
								psi_up(a, i) += U(x, mu, i, j)
										* psi_in(x + mu, l, a, j);
								psi_dw(a, i) += conj(U(x - mu, mu, j, i))
										* psi_in(x - mu, l, a, j);
							}
							psi_out(x, l, a, i) += kappa5
									* (psi_dw(a, i) + psi_up(a, i));
							psi_out(x, l, Gamma_idx[mu][a], i) +=
									(kappa5 * sign) * Gamma_val[mu][a]
											* (psi_dw(a, i) - psi_up(a, i));
				...
				if (l < L5 - 1)
					for (a = 0; a < nspin; a++)
						for (int i = 0; i < nc; i++) {
							psi_out(x, l, a, i) += kappa5
									* psi_in(x, l + 1, a, i);
							psi_out(x, l, Gamma5_idx[a], i) -= kappa5
									* Gamma5_val[a] * psi_in(x, l + 1, a, i);
						}
				else
					for (a = 0; a < nspin; a++)
						for (int i = 0; i < nc; i++) {
							psi_out(x, L5 - 1, a, i) += kappaf
									* psi_in(x, 0, a, i);
							psi_out(x, L5 - 1, Gamma5_idx[a], i) -= kappaf
									* Gamma5_val[a] * psi_in(x, 0, a, i);
						}
				if (l > 0)
					for (a = 0; a < nspin; a++)
						for (int i = 0; i < nc; i++) {
					...
					for (a = 0; a < nspin; a++)
						for (int i = 0; i < nc; i++) {
							psi_out(x, 0, a, i) += kappaf
									* psi_in(x, L5 - 1, a, i);
							psi_out(x, 0, Gamma5_idx[a], i) += kappaf
									* Gamma5_val[a] * psi_in(x, L5 - 1, a, i);
						}
