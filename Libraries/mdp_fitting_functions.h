...
float golden_rule(float (*fp)(float*, mdp_int, void*), float &xmin, float ax,
		float bx, float cx, float tol = 0.001, mdp_int niter = 100,
		void *dummy = 0) {
	static const float CGOLD = 0.3819660;
	mdp_int iter;
	float a, b, d = 0, etemp, fu, fv, fw, fx, p, q, r, tol1, tol2, u, v, w, x,
			xm;
	float e = 0.0;
	a = (ax < cx ? ax : cx);
	b = (ax > cx ? ax : cx);
...
float BLMaux(float *x, Measure *y, mdp_int i_min, mdp_int i_max, float *a,
		float *a0, mdp_matrix &sigma, int ma, mdp_matrix &alpha,
		mdp_matrix &beta, BLM_function func, float h, void *junk) {
	mdp_int i, j, k;
...
float BaesyanLevenbergMarquardt(float *x, Measure *y, mdp_int i_min,
		mdp_int i_max, float *a, int ma, mdp_matrix &covar, BLM_function func,
		float h = 0.001, mdp_int nmax = 1000, void *junk = 0) {
	...
	chi_square = BLMaux(x, y, i_min, i_max, a, a0, sigma, ma, alpha, beta, func,
			h, junk);
	...
	for (n = 0; n < nmax; n++) {
		...
		chi_square = BLMaux(x, y, i_min, i_max, atry, a0, sigma, ma, covar,
				oneda, func, h, junk);
