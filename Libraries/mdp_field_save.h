...
/// Auxiliary function
bool mdp_default_user_write(FILE *fp, void* p, mdp_int psize,
...
template<class T>
bool mdp_field<T>::save(string filename, int processIO, mdp_int max_buffer_size,
		bool save_header, mdp_int skip_bytes,
	...
	if (ME == processIO) {
		...
		for (idx_gl = 0; idx_gl < nvol_gl; idx_gl++) {
			...
			if (process != NOWHERE) {
				if (user_write) {
					if (!user_write(fp, short_buffer, field_components * Tsize,
