...
public:
	static void smear(fermi_field& psi, gauge_field& U, coefficients& coeff) {

		if (coeff.has_key("factor"))
			error(
					"WupperthalSmearing::smear()\nCoefficient 'factor' undefined.");
...
/// smears a propagator
void smearSink(fermi_propagator &S, gauge_field &U,
		void (*smf)(fermi_field&, gauge_field&, coefficients&),
		coefficients& coeff) {
