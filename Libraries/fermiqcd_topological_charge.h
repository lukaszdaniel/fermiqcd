...
class ApeSmearing {
public:
	static void smear(gauge_field &U, mdp_real alpha = 0.7, int iterations = 20,
			int cooling_steps = 10) {
...
float topological_charge_vtk(gauge_field &U, string filename, int t = -1) {
	...
	cumulate_field(Q, filename).save_vtk(
			filename.replace(filename.rfind("."), 1, ".sum."), t);
