...
public:
	...
	;
	void allocate_fermi_propagator(mdp_lattice &mylattice, int nc_, int nspin_ =
			4) {
		deallocate_field();
		nspin = nspin_;
		nc = nc_;
	...
	friend void generate(fermi_propagator &S, gauge_field &U,
			...
			int max_steps = 2000,
			void (*smf)(fermi_field&, gauge_field&, coefficients&)=0,
			coefficients smear_coeff = coefficients(), int comp = 0) {
		...
		for (b = 0; b < psi.nspin; b++)
			for (j = 0; j < psi.nc; j++) {
				...
				stats = mul_invQ(chi, psi, U, coeff, absolute_precision,
				...
				cout << "Statistics: residue=" << stats.residue << ", steps="
						<< stats.steps << ", time=" << stats.time << '\n';
			}
			...
			for (int b = 0; b < 4; b++) {
				forallsites(x)
				{
					...
					for (int mu = 0; mu < 4; mu++)
						V(x) += U(x, mu) * S(x + mu, a, b)
								+ hermitian(U(x - mu, mu)) * S(x - mu, a, b);
				}
				V.update();
				forallsites(x)
				{
