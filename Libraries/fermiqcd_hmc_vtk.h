...
public:
	...
	mdp_real compute_action(GaugeClass &U, GaugeClass &V, FermiClass &F) {
		...
		double c = coeff["beta"] * (U.ndim * (U.ndim - 1) * U.lattice().nvol_gl)
				/ 2;
		...
		if (coeff["dynamical_fermions"] > 0) {
			...
			CG2::inverter(inverse_F, F, V, coeff,
					...
					coeff["cg_relative_precision"], coeff["cg_max_steps"],
					true);
			action_fermi += real_scalar_product(F, inverse_F);
		}
		return action_gauge + action_fermi;
	}
	void compute_fields_evolution(GaugeClass &U, GaugeClass &p_U,
			GaugeClass &f_U, FermiClass &F, FermiClass &p_F, FermiClass &f_F) {
	...
	void compute_force(GaugeClass &U, GaugeClass &f_U, FermiClass &F,
			FermiClass &f_F) {
		...
		forallsites(x)
		{
			for (int mu = 0; mu < U.ndim; mu++) {
				...
				for (int nu = 0; nu < U.ndim; nu++)
					if (nu != mu)
						staple = staple
								+ U(x + mu, nu) * Udag(x + nu, mu) * Udag(x, nu)
								+ Udag(x + mu - nu, nu) * Udag(x - nu, mu)
										* U(x - nu, nu);
		...
		if (coeff["dynamical_fermions"] > 0) {
			...
			compute_effective_links(U, V);
			CG2::inverter(sol, F, V, coeff, coeff["cg_absolute_precision"],
					coeff["cg_relative_precision"], coeff["cg_max_steps"],
					true);
	...
	void compute_fermion_forces(GaugeClass &U, GaugeClass &f_U, FermiClass &sol,
			FermiClass &psol) {
			...
			forallsites(x)
			{
				for (int mu = 0; mu < U.ndim; mu++) {
					...
					for (int a = 0; a < fnc; a++) {
						for (int b = 0; b < fnc; b++) {
							stemp =
									hermitian(spinor(psol, x, a))
											* ((1 - Gamma[mu])
													* spinor(sol, x + mu, b));
							dum -= stemp(0, 0) * U(x, mu) * S[b] * Utr(x, mu)
									* S[a];
							stemp = hermitian(spinor(psol, x + mu, a))
									* ((1 + Gamma[mu]) * spinor(sol, x, b));
							dum += stemp(0, 0) * S[b] * Udagtr(x, mu) * S[a]
									* Udag(x, mu);
						}
					}
					dum -= hermitian(dum);
