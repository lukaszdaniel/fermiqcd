...
template<class T>
void mdp_field<T>::load(char filename[], int processIO, mdp_int max_buffer_size,
		char *header, mdp_int header_size,
		mdp_int (*sort_x)(mdp_lattice&, mdp_int), int auto_switch_endianess) {
	...
	if (ME == processIO) {
		...
		if (strcmp(header, "NATIVE") == 0) {
			error("NATIVE HEADER IN DEPRECATED FUNCTION NOT SUPPORTED ANYMORE");
		} else if (header != 0 && strcmp(header, "NOHEADER") != 0) {
			if (fread(header, sizeof(char), header_size, fp) != header_size)
		...
		for (idx_gl = 0; idx_gl < nvol_gl; idx_gl++) {
			...
			if (process != NOWHERE) {
				if (sort_x != 0)
					if (fseek(fp,
							sort_x(lattice(), idx_gl) * Tsize * field_components
									+ header_size, SEEK_SET) < 0)
			...
			if ((process != NOWHERE) && (process != processIO)) {
				for (k = 0; k < field_components; k++)
					large_buffer(process, buffer_size[process], k) =
							short_buffer[k];
				buffer_size[process]++;
				if (buffer_size[process] == max_buffer_size) {
					mpi.put(&(large_buffer(process, 0, 0)),
							max_buffer_size * field_components, process,
							request);
					mpi.wait(request);
					buffer_size[process] = 0;
				}
			...
			if (process == processIO) {
				for (k = 0; k < field_components; k++)
					*(m + lattice().lg[idx_gl] * field_components + k) =
							short_buffer[k];
			}
		}
		delete[] buffer_size;
		...
		for (idx_gl = 0; idx_gl < nvol_gl; idx_gl++) {
			...
			if ((buffer_size == max_buffer_size)
				...
				mpi.get(&(local_buffer(0, 0)), buffer_size * field_components,
						processIO);
				for (idx = 0; idx < buffer_size; idx++)
					for (k = 0; k < field_components; k++)
						*(m + local_index[idx] * field_components + k) =
								local_buffer(idx, k);
				buffer_size = 0;
			}
		}
...
template<class T>
void mdp_field<T>::save(char filename[], int processIO, mdp_int max_buffer_size,
		char *header, mdp_int header_size,
		mdp_int (*sort_x)(mdp_lattice&, mdp_int), char *mode) {
	mdp_int idx_gl, nvol_gl = lattice().nvol_gl, k;
	double mytime = mpi.time();
	if (ME == processIO) {
		mdp_int *buffer_size = new mdp_int[Nproc];
		...
		if (strcmp(header, "NATIVE") == 0) {
			error(
					"NATIVE HEADER IN DEPRECATED FUNCTION NOT SUPPORTED ANY MORE");
		} else if (header != 0 && strcmp(header, "NOHEADER") != 0) {
			if (fwrite(header, sizeof(char), header_size, fp) != header_size)
		...
		for (idx_gl = 0; idx_gl < nvol_gl; idx_gl++) {
			...
			if ((process != NOWHERE) && (process != processIO)) {
				...
				for (k = 0; k < field_components; k++)
					short_buffer[k] = large_buffer(process, buffer_ptr[process],
							k);
				buffer_ptr[process]++;
			...
			if (process == processIO) {
				for (k = 0; k < field_components; k++)
					short_buffer[k] = *(m
							+ lattice().lg[idx_gl] * field_components + k);
			}
			if (process != NOWHERE) {
				if (sort_x != 0)
					if (fseek(fp,
		...
		for (idx_gl = 0; idx_gl < nvol_gl; idx_gl++) {
			...
			if ((buffer_size == max_buffer_size)
				...
				for (idx = 0; idx < buffer_size; idx++)
					for (k = 0; k < field_components; k++)
						local_buffer(idx, k) = *(m
								+ local_index[idx] * field_components + k);
