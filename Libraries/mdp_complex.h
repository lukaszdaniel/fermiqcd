...
public:
	...
	inline friend mdp_complex pow(const mdp_complex& c, mdp_real z) {
		mdp_real r = pow((double) abs(c), (double) z), p = (mdp_real) z
				* phase(c);
	...
	inline friend mdp_complex sin(const mdp_complex& c) {
		return mdp_complex(cosh(c.imag()) * sin(c.real()),
				sinh(c.imag()) * cos(c.real()));
	}

	inline friend mdp_complex cos(const mdp_complex& c) {
		return mdp_complex(cosh(c.imag()) * cos(c.real()),
				-sinh(c.imag()) * sin(c.real()));
	}
...
inline mdp_complex operator/(const mdp_complex& a, const mdp_complex& b) {
	...
	return mdp_complex((a.real() * b.real() + a.imag() * b.imag()) / den,
