...
void dft(mdp_complex *fft_f, mdp_complex *f, mdp_int n, double sign,
	...
	for (i = 0; i < n; i++) {
		...
		for (j = 0; j < n; j++)
			fft_f[offset + coeff * i] += f[offset + coeff * j]
					* pow(phase, i * j);
...
void fermi_field_fft_t(fermi_field& psi_out, fermi_field& psi_in, int sign) {
...
void mdp_complex_field_fft(int t, mdp_field<mdp_complex>& psi_out,
		mdp_field<mdp_complex>& psi_in, int sign) {
...
void mdp_complex_field_fft(mdp_field<mdp_complex>& psi_out,
		mdp_field<mdp_complex>& psi_in, int sign, bool ttime = false) {
	for (int t = 0; t < psi_in.lattice().size(0); t++)
...
int test1() {
	...
	for (i = 0; i < n; i++) {
		if (tmp += abs(chk[i] - in[i]) > 0.00001)
			printf("(%f, %f) (%f, %f) (%f, %f)\n", real(in[i]), imag(in[i]),
					real(chk[i]), imag(chk[i]), real(out[i]), imag(out[i]));
	};
	printf("%f\n", tmp / n);
	return 0;
}
