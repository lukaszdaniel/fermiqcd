...
\begin{verbatim}
g++ prg.cpp -I../Libraries -o prg.exe -O3 -DUSE_SINGLE_PRECISION
\end{verbatim}

will compile the algorithms to use single precision
\begin{verbatim}
g++ prg.cpp -I../Libraries -o prg.exe -O3
\end{verbatim}

will compile the algorithms to use O3 optimizations
\begin{verbatim}
...
int main(int argc, char** argv) {
   ...
   declare_base_matrices(''FERMIQCD'');
   constexpr Box box={16,8,8,8}
   mdp_lattice mylattice(box);
   site x(mylattice);
   forallsites(x)
      std::cout<<mylattice.random(x).plain()<<std::endl;
   mdp.close_wormholes();
   return 0;
}
\end{verbatim}
...
int main(int argc, char** argv) {
   ...
   int niter=10;
   int mu=0, nu=1;
   Path path={{+1,mu},{+1,mu},{+1,nu},{-1,mu},{-1,nu},{-1,nu}};
   for(int k=0; k<100; k++) {
      WilsonGaugeAction::heatbath(U,gauge,niter);
      U.save(string(''gauge.'')+tostring(k)+string(''.mdp''));
      mdp << average_path(U,path) << endl;
   }
   mdp.close_wormholes();
   return 0;
}
...
mdp_complex average_path(gauge_field &U, const Path &d) {
   ...
   for(int i=0; i<d.length(); i++) {
     ...
     if(i==0) forallsites(x) psi(x)=U(x,orientation,direction);
     else forallsites(x) psi(x)*=U(x,orientation,direction);
     if(i<d.length()-1) psi.shift(orientation,direction);
