...
int main(int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; i++)
  {
    if (strncmp(argv[i], "-verbose", 8) == 0)
      ...
      sscanf(argv[i], "-hmass=%f", &hmass);
    else if (strncmp(argv[i], "-u0", 3) == 0)
      sscanf(argv[i], "-u0=%f", &u0);
    else if (strncmp(argv[i], "-ap", 3) == 0)
      sscanf(argv[i], "-ap=%f", &ap);
#else
    else if (strncmp(argv[i], "-lmass", 6) == 0)
      sscanf(argv[i], "-lmass=%lf", &lmass);
    else if (strncmp(argv[i], "-hmass", 6) == 0)
      sscanf(argv[i], "-hmass=%lf", &hmass);
    else if (strncmp(argv[i], "-u0", 3) == 0)
      sscanf(argv[i], "-u0=%lf", &u0);
    else if (strncmp(argv[i], "-ap", 3) == 0)
      sscanf(argv[i], "-ap=%lf", &ap);
#endif
    else if (strncmp(argv[i], "-bicguml", 7) == 0)
      inverter = bicguml;
  ...
  nc = (mdp_suint)sqrt((1.0 * header.bytes_per_site) / (4 * sizeof(mdp_complex)));
  ...
  {
  case bicguml:
    mdp << "Using bicg uml (milc) inverter\n";
    break;
  case minres:
