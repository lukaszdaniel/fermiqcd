...
int main(int argc, char **argv)
{
  ...
  constexpr Box L = {32, 8, 8, 8};  // lattice volume
  constexpr mdp_suint nc = 3;       // SU(n) gauge group
  constexpr mdp_suint N = 100;      // number of gauge configurations
  ...
  mdp_array<mdp_real, 1> C(L[0]);   // make a 1D array of 32 mdp_real elements
  fermi_field psi(lattice, nc);     // make a fermionic field
  fermi_field chi(lattice, nc);     // make another fermionic field
  mdp_site x(lattice);              // variable to loop on the lattice
  set_hot(U);                       // make a random gauge configuration
  for (mdp_uint t = 0; t < L[0]; t++)
    C(t) = 0; // zero the output
  for (mdp_suint k = 0; k < N; k++)
  {                                               // loop over MCMC
    WilsonGaugeAction::heatbath(U, gauge, 10);    // do 10 MCMC steps
    for (mdp_suint alpha = 0; alpha < 4; alpha++) // source spin index
      ...
      { // source color index
        ...
        forallsites(x)
        { // contract sink
          for (mdp_suint beta = 0; beta < 4; beta++)
  ...
  for (mdp_uint t = 0; t < L[0]; t++) // print output
    mdp << "C(" << t << ")=" << C(t) << "\n";
